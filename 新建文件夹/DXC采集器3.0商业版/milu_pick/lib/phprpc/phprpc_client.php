...
class _PHPRPC_Client {
   ...
   function _connect() {
		...
		if(!function_exists('fsockopen') && !function_exists('pfsockopen')) die('fsockopen和pfsockopen至少开启一个');
    ...
    function _post($request_body) {
		...
		if(function_exists('gzinflate')) $gzip_str = "Accept-Encoding: gzip,deflate\r\n";//有些虚拟主机不开启 gzinflate函数
        $request =
