...
function recommendupdate(n) {
	if(getcookie('recommend')) {
		var objv = n > 0 ? Scratching('recommendv_add') : Scratching('recommendv_subtract');
		objv.style.display = '';
		objv.innerHTML = parseInt(objv.innerHTML) + 1;
		setTimeout(function () {
			Scratching('recommentc').innerHTML = parseInt(Scratching('recommentc').innerHTML) + n;
			Scratching('recommentv').style.display = 'none';
		}, 1000);
		setcookie('recommend', '');
	}
}
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button" type="button" value="确定" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button" type="button" value="确定" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">取消</a></dd></div>'
			}
...
function(factory, global) {
    ...
    HideShowPassword.prototype = {
        ...
        ifCurrentOrNot: function(ifCurrent, ifNot) {
            ...
            $.each(this.options.states,
            function(thisKey, state) { (currentKey === thisKey ? ifCurrent: ifNot)(state)
            })
        },
        initInnerToggle: function(el, options) {
            ...
            elCSS["padding" + attachment.replace(/./,
            function(m) {
            ...
            if (options.hideToggleUntil) {
                toggle.hide();
                el.one(options.hideToggleUntil,
                function() {
                    toggle.show()
                })
            }
