...
<style>.ainuo_audio .audiojs{width:100%;}.ainuo_audio .audiojs .play-pause{width:40px;}.ainuo_audio .audiojs .scrubber{width:48%;}</style>
...
<div class="postlist ainuoloadmore" id="ainuoloadmore">
   ...
   <div class="{if $post['first']}plc{else}plcnotfirst{/if}" id="pid$post[pid]" {if $post[first]} style="border-bottom:none;"{/if}>
       <!--{if $post[first]}--><!--From ww w.moq u8 .com -->
       	...
       	<div class="ainuo_vtitle cl">
            ...
            <div class="newst cl">
            	<span class="bankuai"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]">$_G['forum'][name]</a></span>
            	<a href="{if $_G[uid] == $post[authorid]}home.php?mod=space&uid={$post[authorid]}&do=profile&mycenter=1{else}home.php?mod=space&uid={$post[authorid]}&do=profile{/if}" {if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>
                <span class="dat">$post[dateline]</span>
            	<span class="y">$allreply $alang_reply</span>
            </div>
            <!--{/if}-->
            ...
            <div class="info cl">
            	...
            	<!--{if in_array($thread['displayorder'], array(1, 2, 3, 4))}-->
                    <span class="displayorder">$_G[setting][threadsticky][3-$thread[displayorder]]</span>
                <!--{/if}-->
                <!--{if $thread['digest'] > 0}-->
                    {lang digest}
                <!--{/if}-->
                ...
                <!--{if $_G['forum_thread'][heatlevel]}-->
                    <span class="heats">{lang heats}: $_G['forum_thread']['heats']</span>
                <!--{/if}-->
                <!--{if $_G['forum_thread']['closed'] == 1}-->
                    <span class="closed">{lang close}</span>
                <!--{/if}-->
                <!--{if $_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}-->
                    <span class="flei">{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}</span>
                <!--{/if}-->
                <!--{if $threadsorts && $_G['forum_thread']['sortid']}-->
                    <span class="flei">{$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]}</span>
                <!--{/if}-->
                <!--{if $post['warned']}-->
                    <span class="closed">{lang warn_get}</span>
                <!--{/if}-->
                <span class="bankuai"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]">$_G['forum'][name]</a></span>
            	<span class="y" style="margin-left:10px;">$_G[forum_thread][views] $alang_view</span><span class="y">$allreply $alang_reply</span>
   ...
   {if $post[first]}
		...
		<div class="reply_bot cl">
            ...
            {if !$allreply}
            <div class="ccon cl">
            	<p style="margin:0 20px;">$alang_replysofa</p>
            </div>
            {/if}
		</div>
   {/if}
...
<!--{if in_array($_G['fid'], $ainuo_array_activitylist)}-->
	<!--{template forum/forumdisplay_fastpost_activity}-->
<!--{else}-->
...
$('.favbtn').on('click', function() {
	...
	.success(function(s) {
		if(s.lastChild.firstChild.nodeValue.indexOf("$alang_haveshoucang") >= 0){
			...
			$(".favbtn .iconfont").removeClass("icon-favor");
			$(".favbtn .iconfont").addClass("icon-favorfill");
			Zepto('.ainuooverlay').remove();
...
function listzan(apid) {
	...
	.success(function(s) {
		if(s.lastChild.firstChild.nodeValue.indexOf("$alang_nbntp") >= 0){
			popup.close();
			Zepto.toast('$alang_nbndzo',1500,'toast');
		}else if(s.lastChild.firstChild.nodeValue.indexOf("$alang_nyjtp") >= 0){
			popup.close();
			Zepto.toast('$alang_nyjzg',1500,'toast');
		}else{
			popup.close();
			Zepto.toast('$alang_dzcg',1500,'toast');
...
$('.ainuozan').on('click', function() {
	...
	.success(function(s) {
		if(s.lastChild.firstChild.nodeValue.indexOf("$alang_yipingjia") >= 0){
			Zepto('.ainuooverlay').remove();
			Zepto.toast('$alang_dianzanguo',1500,'toast');
			...
			$(".ainuozan .iconfont").removeClass("icon-appreciate");
			$(".ainuozan .iconfont").addClass("icon-appreciatefill");
			Zepto('.ainuooverlay').remove();
			Zepto.toast('$alang_zanjiayi2',1500,'toast');
		}else if(s.lastChild.firstChild.nodeValue.indexOf("\u60a8\u76ee\u524d\u6ca1\u6709\u6743\u9650\u8bc4\u4ef7\u6b64\u4e3b\u9898") >= 0){
			Zepto('.ainuooverlay').remove();
			Zepto.toast('\u8bf7\u5148\u52a0\u5165\u8be5\u7fa4\u7ec4',1500,'toast');
		}else if(s.lastChild.firstChild.nodeValue.indexOf("\u60a8\u4e0d\u80fd\u8bc4\u4ef7\u81ea\u5df1\u7684\u5e16\u5b50") >= 0){
			Zepto('.ainuooverlay').remove();
			Zepto.toast('\u60a8\u4e0d\u80fd\u7ed9\u81ea\u5df1\u70b9\u8d5e',1500,'toast');
		}else{
			Zepto('.ainuooverlay').remove();
...
function ainuoajax(aurl){
	...
	.error(function() {
		Zepto('.ainuooverlay').remove();
		Zepto.toast('$alang_pleasetye',1500,'toast');
	});
	return false;
}
</script>
