...
.postalbum_i { margin-bottom:-3px; max-width:100%; vertical-align:middle; visibility:hidden; }
	...
	<div class="postalbum">
		<div class="postalbum_h">
			<a href="javascript:;" onclick="history.go(-1)" class="postalbum_h_back">{lang return}</a>
...
(function() {
	...
	jQuery('.postalbum').css({'display':'block', 'height':height + 'px'});
	jQuery('.postalbum_u').css({'height':height + 'px', 'width':width + 'px'});
	jQuery('.postalbum_i').css({'max-height':'100%', 'visibility':'visible'});
	if(support3d) {
		jQuery('.postalbum_c').css({'line-height':height + 'px', 'width':width * count + 'px'});
		slidmoving('-' + curkey * width);
	} else {
		jQuery('.postalbum_c').css({'display':'block', 'height':height * count + 'px'});
		jQuery('.postalbum_c').css({'top': '-' + curkey * height + 'px'});
	}
	...
	var curkeyimg = jQuery('#img_' + curkey);
	...
	var timeoutid = null;
	var imgtouchpos = {};
	jQuery('.postalbum_u').on('touchstart', '.postalbum_i', function(e) {
		if(!imgmovestatus) {
			return;
		}
		e = mygetnativeevent(e);
		...
		imgchange(jQuery('#img_' + curkey), imgscale, newscalex, newscaley);
	...
	function fiximgmove(imgobj) {
		...
		if(movey > 0) {
			var yoffset = offset.top - jQuery('.postalbum').offset().top;
			if(yoffset > 0) {
	...
	touchaction(jQuery('.postalbum'), '.postalbum_u', function(swipedire, touchevent) {
		if(imgmovestatus) {
			return;
		}
		switch(swipedire) {
			case 'left':
				if(curkey >= count - 1) {
					...
					for(var i=0; i<3; i++) {
						if(!jQuery('#img_' + (curkey + i)).attr('src')) {
							jQuery('#img_' + (curkey + i)).attr('src', jQuery('#img_'+(curkey + i)).attr('zsrc'));
						}
					}
					curkey++;
					if(support3d) {
	...
	function slidmoving(distx) {
		jQuery('.postalbum_c').css('-webkit-transform', 'translate3d('+ distx + 'px, 0, 0)');
		jQuery('.postalbum_c').css('-moz-transform', 'translate3d('+ distx + 'px, 0, 0)');
		jQuery('.postalbum_c').css('-o-transform', 'translate3d('+ distx + 'px, 0, 0)');
		jQuery('.postalbum_c').css('transform', 'translate3d('+ distx + 'px, 0, 0)');
		return true;
	}
