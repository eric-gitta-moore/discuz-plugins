...
<style>
.ainuo_vtitle .tit h1{ font-size:16px;}
</style>
...
<div class="postlist ainuoloadmore" id="ainuoloadmore">
       ...
       <!--{if $post[first]}-->
       	...
       	<div class="ainuo_vtitle cl">
            ...
            <div class="newst cl">
                ...
                <span class="dat" style="margin:0; display:none;"> {lang poston} $post[dateline]</span>
   ...
   {if $post[first]}
        ...
        <!--{eval $avideo_hotlist = DB::fetch_all("SELECT * FROM ".DB::table('forum_thread')." where fid=".$_G[fid]." and tid != ".$_G[tid]." and displayorder >= 0 ORDER BY views DESC limit 5");}-->
        <!--{if $avideo_hotlist}-->
            <div class="recmend_video cl">
            	<div class="atit cl">$nh_hotvideo<a href="forum.php?mod=forumdisplay&fid=$_G['fid']&filter=heat&orderby=heats">$alang_more</a></div>
                <ul>
                <!--{loop $avideo_hotlist $thread}-->
                	<li><a href="forum.php?mod=viewthread&tid=$thread[tid]"><div class="xl cl"><i class="iconfont icon-videofill"></i>$thread[subject]</div><p><span>$thread[views] $nh_playnum</span><span>$thread[replies] $nh_replynum</span></p></a></li>
                <!--{/loop}-->
                </ul>
            </div>
            <div class="grey_line cl"></div>
...
<!--{subtemplate forum/forumdisplay_fastpost_video}-->
