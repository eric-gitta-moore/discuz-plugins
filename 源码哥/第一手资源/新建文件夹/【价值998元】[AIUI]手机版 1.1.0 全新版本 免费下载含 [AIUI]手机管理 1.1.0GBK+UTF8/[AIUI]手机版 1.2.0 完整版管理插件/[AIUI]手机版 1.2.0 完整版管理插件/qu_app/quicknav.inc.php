...
if(submitcheck('quicknav_submit')){

	if(is_array($_POST['delete'])) {
		if($deleteids = dimplode($_POST['delete'])) {
			DB::query("DELETE FROM ".DB::table('qu_appquicknav')." WHERE id IN ($deleteids)");
		}
	}
	...
	if(is_array($_POST['title']) && $_POST['title']) {
		...
		foreach($_POST['title'] as $key => $val) {
			DB::update('qu_appquicknav', array(
	...
	if(is_array($_POST['titlenew'])) {
		foreach($_POST['titlenew'] as $key => $val) {
			$data = array(
				...
				'url' => $_POST['urlnew'][$key],
				'disable' => intval($_POST['disablenew'][$key])
			);
			DB::insert('qu_appquicknav', $data);
		}
	}
	$_G['cache']['ainuodata'] = '';
	savecache('ainuodata', $_G['cache']['ainuodata']);
	cpmsg('plugins_setting_succeed', 'action=plugins&operation=config&do='.$pluginid.'&identifier=qu_app&pmod=quicknav', 'succeed');
}else{
	...
	$total_num = DB::result_first("SELECT count(*) FROM ".DB::table('qu_appquicknav')." WHERE 1 $add_sql");
	$pageurl = 'action=plugins&operation=config&do='.$pluginid.'&identifier=qu_app&pmod=quicknav'.$add_page;
	$multipage = multi($total_num,$perpage,$page,"$pageurl",100);
	
	$floors = '';
	$query = DB::query("SELECT * FROM ".DB::table('qu_appquicknav')." WHERE 1 $add_sql ORDER BY displayorder ASC,id ASC LIMIT $start_limit,$perpage");
	while($result = DB::fetch($query)) {
		$floors .= showtablerow('', array('class="td25"', 'class="td25"','class="td23"', 'class="td23"', 'class="td25"',''), array(
			...
			"<input type=\"text\" class=\"txt\" name=\"url[$result[id]]\" style=\"width:360px\" value=\"".$result['url']."\">",
	...
	[1, \'<input type="text" class="txt" name="urlnew[]" style=\"width:360px\" value="">\', \'\'],
	[1, \'<input type="checkbox" class="checkbox" name="disablenew[]" size="2" value="">\', \'td23\'],
	],
	];
	</script>
	';
	showformheader('plugins&operation=config&do='.$pluginid.'&identifier=qu_app&pmod=quicknav');
	showtableheader();
	showtitle($langvars['nav_setting']);
	showsubtitle(array($langvars['nav_del'], $langvars['nav_order'], $langvars['nav_title'],$langvars['nav_pic'],$langvars['nav_bgcolor'], $langvars['nav_url'],$langvars['nav_disable'],$langvars['nav_preview']));
	echo $floors;
	echo '<tr><td class="td25"></td><td colspan="10"><div><a href="###" onclick="addrow(this, 0)" class="addtr">'.$langvars['nav_add_new'].'</a></div></td>';
	echo '<input type="hidden" name="hid" value="'.$id.'" />';
	showsubmit('','','','<input type="submit" class="btn" name="quicknav_submit" value="'.cplang('submit').'" />',$multipage);
	showtablefooter();
	showformfooter();
