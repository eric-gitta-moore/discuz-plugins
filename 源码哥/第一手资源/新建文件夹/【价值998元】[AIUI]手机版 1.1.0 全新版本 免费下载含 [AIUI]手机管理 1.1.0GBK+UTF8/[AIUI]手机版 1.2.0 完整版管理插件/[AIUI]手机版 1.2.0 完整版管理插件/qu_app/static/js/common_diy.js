...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('您确定要转换模块的类型从 '+ele.options[0].innerHTML+' 到 '+ele.options[ele.selectedIndex].innerHTML)) {
			$.ajax({
				type : 'GET',
				url : 'portal.php?mod=portalcp&ac=block&op=convert&bid='+bid+'&toblockclass='+ele.value,
				dataType : 'xml'
			}).success(function(s) {
				$('.dialogbox .blockshow').html(s.lastChild.firstChild.nodeValue);
			});
		} else {
			ele.selectedIndex = 0;
		}
...
function block_get_setting(classname, script, bid) {
	$.ajax({
		type : 'GET',
		url : 'portal.php?mod=portalcp&ac=block&op=setting&bid='+bid+'&classname='+classname+'&script='+script+'&inajax=1',
		dataType : 'xml'
	}).success(function(s) {
		$('.dialogbox #tbody_setting').html(s.lastChild.firstChild.nodeValue);
	});
}
function block_show_thumbsetting(classname, styleid, bid) {
	$.ajax({
		type : 'GET',
		url : 'portal.php?mod=portalcp&ac=block&op=thumbsetting&classname='+classname+'&styleid='+styleid+'&bid='+bid+'&inajax=1',
		dataType : 'xml'
	}).success(function(s) {
...
function showBlockSummary() {
	$F('block_sumamry_content').style.display='';
	$F('a_summary_show').style.display='none';
	$F('a_summary_hide').style.display='';
	return false;
}
...
var Util = {
	...
	hide:function (ele){
		if (typeof ele == 'string') {ele = $F(ele);}
	...
	toggleEle : function (ele) {
		ele = (typeof ele !='object') ? $F(ele) : ele;
		if (!ele) return false;
		var value = this.getFinallyStyle(ele,'display');
		if (value =='none') {
			this.show(ele);
			this.hide($F('uploadmsg_button'));
		} else {
			this.hide(ele);
			this.show($F('uploadmsg_button'));
		}
	},
	...
	fade : function(obj,timer,ftype,cur,fn) {
		...
		obj = typeof obj == 'string' ? $F(obj) : obj;
		if (!obj) return false;
...
(function(){
	...
	Drag.prototype = {
		...
		_getMinDistance : function (ele, max) {
			...
			var _clientY = parseInt(this.dragObj.style.top);
			var id;
			var isTabInTab = Util.hasClass(this.dragObj, this.tabClass) && Util.hasClass($F(ele.name).parentNode.parentNode, this.tabClass);
			if (ele instanceof Frame || ele instanceof Tab) {
				if (ele.moveable && !isTabInTab) {
					...
					var isBlock = Util.hasClass(this.dragObj, this.blockClass) && Util.hasClass($F(ele.name).parentNode, this.moveableColumn);
		...
		toggleContent : function (e) {
			...
			if (obj instanceof Block || obj instanceof Tab) {
				display = $F(id+'_content').style.display;
				Util.toggleEle(id+'_content');
			} else {
				var col = obj.columns;
				for (var i in col) {
					display = $F(i).style.display;
					Util.toggleEle($F(i));
				}
			}
		...
		checkEdit : function (ele) {
			...
			if (!$F(id+'_edit')) {
				...
				ele.appendChild(dom);
				$F(id+'_edit_menu').onclick = function (e){Drag.prototype.toggleMenu.call(_method, e, this);};
			}
		...
		_initColumn : function (frameObj,frameEle) {
			...
			if (Util.hasClass(frameEle.parentNode.parentNode,this.tabClass)) {
				var col2 = $F(frameEle.id+'_content').children;
		...
		toggleMenu : function (e, obj) {
			...
			if (dom) {
				...
				if (html != '') {
					...
					dom.style.top = objPara.top + objPara.height + 'px';
					dom.style.left = objPara.left - 30 + 'px';
					dom.innerHTML = html;
		...
		_getMenuHtml : function (e,obj) {
			...
			if (Util.hasClass($F(id),this.tabClass) && typeof this.menu['tab'] == 'object') html += this._getMenuHtmlLi(id, this.menu['tab']);
			if (Util.hasClass($F(id),this.frameClass) && typeof this.menu['frame'] == 'object') html += this._getMenuHtmlLi(id, this.menu['frame']);
			if (Util.hasClass($F(id),this.blockClass) && typeof this.menu['block'] == 'object') html += this._getMenuHtmlLi(id, this.menu['block']);
		...
		drag : function (e) {
			...
			var _clientX = e.touches[0].pageX;
			var _clientY = e.touches[0].pageY;
			...
			if (Util.hasClass(ele, this.frameClass) || Util.hasClass(ele, this.tabClass)) {
				...
				if (frame) {
					for (var i in frame['columns']) {
						if (frame['columns'][i] instanceof Column) dom.appendChild($F(i));
					}
				} else {
				...
				$F(tab.id+'_content').appendChild(dom);
			} else if (Util.hasClass(ele, this.blockClass)) {
				if ($F(ele.id+'_content')) $F(tab.id+'_content').appendChild($F(ele.id+'_content'));
			}
		},
		_popTabContent : function (tab, ele){
			if (Util.hasClass(ele, this.frameClass) || Util.hasClass(ele, this.tabClass)) {
				Util.removeClass(ele, this.tabActivityClass);
				var eleContent = $F(ele.id+'_content');
				...
				children = '';
				$F(tab.id+'_content').removeChild(eleContent);
			} else if (Util.hasClass(ele, this.blockClass)) {
				if ($F(ele.id+'_content')) Util.show($F(ele.id+'_content'));
				if ($F(ele.id+'_content')) ele.appendChild($F(ele.id+'_content'));
			}
		},
		_initTabActivity : function (ele) {
			if (!Util.hasClass(ele,this.tabClass)) return false;
			var tabs = $F(ele.id+'_title').childNodes;
			var arrTab = [];
			for (var i in tabs) {
				...
				if (Util.hasClass(tabs[i],this.frameClass) || Util.hasClass(tabs[i],this.tabClass)) {
					...
					var frameContent = $F(tabId+'_content');
					if (frameContent) {
				...
				if (frameContent) $F(ele.id + '_content').appendChild(frameContent);
			}
			var len = arrTab.length;
			for (var i = 0; i < len; i++) {
				Util[i > 0 ? 'hide' : 'show']($F(arrTab[i].id+'_content'));
			}
		},
		dragEnd : function (e) {
			e = Util.event(e);
		...
		initMoveableArea : function () {
			...
			for (var i = 0; i < len; i++) {
				...
				el.onmousedown = function (e) {//草泥马老子偏要强制接管滑动事件
					var ev = e;
					Drag.prototype.dragStart.call(_method, e);
					$(el).on('touchmove', function(e) {
						event.preventDefault();
						Drag.prototype.drag.call(_method, e);
					}).on('touchend', function(e) {
						Drag.prototype.dragEnd.call(_method, ev);
						$(el).unbind("touchmove");
					});
				};
			}
			if ($F('contentframe')) $F('contentframe').ondragstart = function (e) {return false;};
		},
		...
		initCommon : function () {
			this.advancedStyleSheet = $F('diy_common');
			this.menu = [];
		},
		initSample : function () {
			this._getMoveableArea();
	...
	DIY.prototype = {
		...
		init : function (mod) {
			...
			if (this.style == '') {
				...
				var href = $F('style_css') ? $F('style_css').href : '';
		...
		initDiyStyle : function (css) {
			var allCssText = css || $F('diy_style').innerHTML;
		...
		initPalette : function (id) {
			...
			$F('repeat_mode').selectedIndex = selectedIndex;
			for (i=0;i<attachment.length;i++) {
				$F('rabga'+i).checked = (bgattachment == attachment[i] ? true : false);
			}
			var flag = '';
			for (i=0;i<position.length;i++) {
				var className = bgposition == position[i] ? 'red' : '';
				$F('bgimgposition'+i).className = className;
			...
			$F('colorValue').value = bgcolor;
			if ($F('cbpb')) $F('cbpb').style.backgroundColor = bgcolor;
			$F('textColorValue').value = bgfontColor;
			if ($F('ctpb')) $F('ctpb').style.backgroundColor = bgfontColor;
			$F('linkColorValue').value = bglinkColor;
			if ($F('clpb')) $F('clpb').style.backgroundColor = bglinkColor;

			Util.show($F('currentimgdiv'));
			Util.hide($F('diyimages'));
			if ($F('selectalbum')) $F('selectalbum').disabled = 'disabled';
			bgimg = bgimg != '' && bgimg != 'none' ? bgimg.replace(/url\(['|"]{0,1}/,'').replace(/['|"]{0,1}\)/,'') : 'static/image/common/nophotosmall.gif';
			$F('currentimg').src = bgimg;
		},
		changeBgImgDiv : function () {
			Util.hide($F('currentimgdiv'));
			Util.show($F('diyimages'));
			if ($F('selectalbum')) $F('selectalbum').disabled = '';
		},
		...
		changeLayout : function (newLayout) {
			...
			var data = $F('layout'+newLayout).getAttribute('data');
			...
			if (newLayoutLength == currentLayoutLength){
				$F('frame1_left').style.width = dataArr[0]+'px';
				$F('frame1_center').style.width = dataArr[1]+'px';
				if (typeof(dataArr[2]) != 'undefined') $F('frame1_right').style.width = dataArr[2]+'px';
			} else if (newLayoutLength > currentLayoutLength) {
				var block = this.getRandomBlockName();
				var dom = document.createElement('div');
				dom.id = 'frame1_right';
				dom.className = drag.moveableColumn + ' z';
				dom.appendChild($F(block));
				$F('frame1').appendChild(dom);
				$F('frame1_left').style.width = dataArr[0]+'px';
				$F('frame1_center').style.width = dataArr[1]+'px';
				dom.style.width = dataArr[2]+'px';
			} else if (newLayoutLength < currentLayoutLength) {
				var _length = drag.data['diypage'][0]['columns']['frame1_right']['children'].length;
				var tobj = $F('frame1_center_temp');
				for (var i = 0; i < _length; i++) {
					var name = drag.data['diypage'][0]['columns']['frame1_right']['children'][i].name;
					if (name.indexOf('temp') < 0) $F('frame1_center').insertBefore($F(name),tobj);
				}
				$F('frame1').removeChild($F('frame1_right'));
				$F('frame1_left').style.width = dataArr[0]+'px';
				$F('frame1_center').style.width = dataArr[1]+'px';
			}

			var className = $F('layout'+this.currentLayout).className;
			$F('layout'+this.currentLayout).className = '';
			$F('layout'+newLayout).className = className;
		...
		changeStyle : function (t) {
			if (t == '') return false;
			$F('style_css').href=STATICURL+t+"/style.css";
		...
		setCurrentDiy : function (type) {
			if (type) {
				$F('diy_tag_'+this.currentDiy).className = '';
				this.currentDiy = type;
				$F('diy_tag_'+this.currentDiy).className = 'activity';
		...
		undo : function () {
			...
			$F('button_redo').className = '';
			if (this.opPointer == 0) {
				$F('button_undo').className = 'unusable';
				drag.isChange = false;
				drag.clearClose();
				return '';
		...
		redo : function () {
			...
			$F('button_undo').className = '';
			if (this.opPointer == this.opSet.length) {
				$F('button_redo').className = 'unusable';
				return '';
		...
		uploadSubmit : function (){
			...
			if (document.uploadpic.albumid != null) document.uploadpic.albumid.value = $F('selectalbum').value;
			return true;
		},
		save : function () {
			return false;
