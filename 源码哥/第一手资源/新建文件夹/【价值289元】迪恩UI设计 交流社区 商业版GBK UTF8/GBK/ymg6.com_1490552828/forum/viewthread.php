...
<div id="pt" class="bm cl">
        <div class="z">
            <a href="./" class="nvhm" title="{lang homepage}"></a><em>&raquo;</em><a href="forum.php">{$_G[setting][navs][2][navname]}</a>$navigation <em>&rsaquo;</em> <a href="forum.php?mod=viewthread&tid=$_G[tid]">$_G[forum_thread][short_subject]</a>
        </div>
    </div>
...
<div class="deanluntan_left">
	<div class="deanluntan_view">
    	<!--版块介绍-->
            <div class="deanbkjs">
            	<div class="deanjstop">
                	<div class="deanjstopl"><!--{if $_G[forum][banner] && !$subforumonly}--><img src="data/attachment/common/$_G[forum][banner]" alt="data/attachment/common/$_G['forum'][name]" /><!--{/if}--></div>
                    <div class="deanjstopr">
                    	<h3><a class="deanbkname" href="forum.php?mod=forumdisplay&fid=$_G[fid]">$_G['forum'][name]</a></h3>
                        <div class="deanjsbottom">
                            <ul>
                                <span>{lang index_today}:</span><span>$_G[forum][todayposts]</span><em>|</em>
                                <span>{lang index_threads}:</span><span>$_G[forum][threads]</span>
                                <div class="clear"></div>
                            </ul>
                        </div>
                        <a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum" id="a_favorite" class="deansc" title="{lang forum_favorite}" onclick="showWindow(this.id, this.href, 'get', 0);">{lang forum_favorite} <strong class="xi1" id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}>(<span id="number_favorite_num">$_G[forum][favtimes]</span>)</strong></a>
                        <div class="deanbanzhu y"><!--{if $moderatedby}-->{lang forum_modedby}: <span class="xi2">$moderatedby</span><!--{/if}--></div>
                        <!--新帖-->
            
          	<a class="deanfatie" href="javascript:;" id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial'"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}">发表新帖</a>
                ...
                <div class="deanjscenter">
                    ...
                    <!--{if $_G['page'] == 1 && $_G['forum']['rules']}-->
                    	<div class="deani"></div>
                        <div id="forum_rules_{$_G[fid]}" class="deanbkms" style="$collapse['forum_rules'];">
	...
	<table cellspacing="0" cellpadding="0">
		<tr>
			...
			<td class="plc ptm pbn vwthd">
				<!--{if !IS_ROBOT}-->
					<div class="y">
						<!--{if $post['invisible'] == 0}--><a href="forum.php?mod=viewthread&action=printable&tid=$_G[tid]" title="{lang thread_printable}" target="_blank"><img src="$_G['style'][styleimgdir]/viewthread/printer.png" alt="{lang thread_printable}" class="vm" /></a>
						<!--{/if}-->
						<a href="forum.php?mod=redirect&goto=nextoldset&tid=$_G[tid]" title="{lang last_thread}"><img src="$_G['style'][styleimgdir]/viewthread/thread-prev.png" alt="{lang last_thread}" class="vm" /></a>
						<a href="forum.php?mod=redirect&goto=nextnewset&tid=$_G[tid]" title="{lang next_thread}"><img src="$_G['style'][styleimgdir]/viewthread/thread-next.png" alt="{lang next_thread}" class="vm" /></a>
					</div>
				<!--{/if}-->
