...
<!--{if $close_leftinfo}-->
<style> 
#postlist .plc {padding:0px;} 
#postlist .plc .side_btn .btn_s_open { margin-left:7px; }
.authi .wk_ckhf { float:left; margin-top:0; line-height:24px; }
</style>
<!--{/if}-->
...
<!--[diy=diy_ad_a]--><div id="diy_ad_a" class="area"></div><!--[/diy]-->
...
<div id="postlist" class="pl bm wk_bm">
...
<!--{if $modmenu['thread']}-->
	<div class="xi2 mbm pbm bbs wk_bbsbj">
...
<!--{eval $nxtpage = $page + 1;}-->
	<div class="pgbtn wk_pgbtn"><a href="forum.php?mod=viewthread&tid=$_G[tid]{if $_GET[authorid]}&authorid=$_GET[authorid]{/if}&page=$nxtpage" hidefocus="true" class="bm_h">{lang next_page_extra}</a></div>
<!--{/if}-->
...
<!--[diy=diy_ad_b]--><div id="diy_ad_b" class="area"></div><!--[/diy]-->
...
<div class="wk_f_sd">
    <div class="wk_kd260 padding15 backfff">
		<a id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang send_posts}" class="wk_tzpost"><i class="icon wk_tzpost_icon1"></i><span>$forum_lang[27]</span></a>
      	<a id="post_reply" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}" class="wk_tzpost"><i class="icon wk_tzpost_icon2"></i><span>$forum_lang[28]</span></a>
		<div class="clear"></div>
    </div>
    <div class="clear"></div>
