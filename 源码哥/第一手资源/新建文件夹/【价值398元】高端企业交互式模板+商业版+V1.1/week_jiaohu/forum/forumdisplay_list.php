...
<div id="threadlist" class="tl bm bmw wk_threadlist"{if $_G['uid']} style="position:relative;"{/if}>
	...
	<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
        <div class="wk_gjhk">
        	<div class="wk_ggxx">
           		<span class="wk_yhg">{lang announcement}&nbsp;: </span><span><!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank" title="{lang announcement} : $announcement[subject]">" $announcement[subject] "</a><!--{else}--><a href="$announcement[message]" target="_blank" title="{lang announcement} : $announcement[subject]">" $announcement[subject] "</a><!--{/if}--></span><span class="wk_fs12">$forum_lang[1] : $announcement[starttime]</span>
       		</div> 
        </div>
    <!--{/if}-->
    
    <div class="th week_box">
        <div class="y">
        ...
        <div class="z">
            ...
            <span class="wk_showmenu">
                <a id="filter_dateline" href="javascript:;" class="showmenu {if $_GET['specialtype']} xw1{/if}" onclick="showMenu(this.id)">
                    <!--{if !$_GET['dateline']}-->
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				...
				<!--{if !$separatepos || !$_G['setting']['forumseparator']}-->
					<tbody id="separatorline" class="emptb"><tr><td class="icon">&nbsp;</td><td class="icn"></td><!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o"></td><!--{/if}--><th></th><!--{if CURMODULE == 'guide'}--><td class="by"></td><!--{/if}--><td class="by"></td><td class="num"></td><td class="by"></td></tr></tbody>
				<!--{/if}-->
						...
						<!--{loop $_G['forum_threadlist'] $key $thread}-->							
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								<tbody id="separatorline">
									<tr class="ts">
										<td class="icon" style="border-left:1px #F0F0F0 solid;">&nbsp;</td><td>&nbsp;</td>
										<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><td>&nbsp;</td><!--{/if}-->
										<th><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}--></th><td>&nbsp;</td><td>&nbsp;</td><td style=" border-right:1px #F0F0F0 solid;">&nbsp;</td>
									</tr>
								</tbody>
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr {eval $i++;} {if ($i % 2 === 0)} {eval $i=0;} class="list_bg2"{/if}>
                                 <td class="icon">
                                        <a href="home.php?mod=space&uid=$thread[authorid]" class="w_uid" c="1"><!--{avatar($thread[authorid],small)}--></a>
                                </td>
									<td class="icn">
									...
									<th class="$thread[folder]">
                                    	<!--{if CURMODULE == 'guide'}-->
                                            <span><a href="forum.php?mod=forumdisplay&fid=$thread[fid]" target="_blank">$forumnames[$thread[fid]]['name']</a></span> 
                                        <!--{/if}-->
                                        ...
                                        <div class="wk_tzlb_yh">
                                            ...
                                            <span>
                                                ...
                                                <span class="pipe"></span>
                                                <a href="{if $thread[digest] != -2 && !$thread[ordertype]}forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost$highlight#lastpost{else}forum.php?mod=viewthread&tid=$thread[tid]{if !$thread[ordertype]}&page={echo max(1, $thread[pages]);}{/if}{/if}">$thread[lastpost]</a></span>
                                        </div>
									</th>
									<td class="by">
										<span class="ico_reply"><a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra"><!--{if $thread[allreplies]}-->$thread[allreplies]<!--{else}-->0<!--{/if}--></a></span>
									</td>
									<td class="num"></td>
									<td class="by">
					...
					<!--{else}-->
						...
						<ul id="waterfall" class="ml waterfall cl">
							...
							<!--{if !$thread['forumstick'] && ($thread['isgroup'] == 1 || $thread['fid'] != $_G['fid'])}-->
								...
								<!--{/if}-->
							<!--{/if}-->
							<!--{eval $waterfallwidth = $_G[setting][forumpicstyle][thumbwidth] + 14; }-->
...
<!--{if !IS_ROBOT}-->
	<div id="filter_special_menu" class="p_pop wk_sub" style="display:none" change="location.href='forum.php?mod=forumdisplay&fid=$_G[fid]&filter='+$('filter_special').value">
		<ul>
	...
	<div id="filter_reward_menu" class="p_pop wk_sub" style="display:none" change="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=specialtype&specialtype=reward$forumdisplayadd[specialtype]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}&rewardtype='+$('filter_reward').value">
		<ul>
    ...
    <div id="filter_dateline_menu" class="p_pop wk_sub" style="display:none">
        <ul>
	...
	<div id="filter_orderby_menu" class="p_pop wk_sub" style="display:none">
		<ul>
...
<!--{if $multipage && $filter != 'hot'}-->
	<!--{if !($_G[forum][picstyle] && !$_G[cookie][forumdefstyle])}-->
		<a class="bm_h wk_fbm_h" href="javascript:;" rel="$multipage_more" curpage="$page" id="autopbn" totalpage="$maxpage" picstyle="$_G[forum][picstyle]" forumdefstyle="$_G[cookie][forumdefstyle]">{lang next_page_extra}</a>
...
<div class="bm bw0 pgs cl">
	...
	<span {if $_G[setting][visitedforums]}id="visitedforumstmp" onmouseover="$('visitedforums').id = 'visitedforumstmp';this.id = 'visitedforums';showMenu({'ctrlid':this.id,'pos':'21'})"{/if} class="pgb"><a href="forum.php">{lang return_index}</a></span>
