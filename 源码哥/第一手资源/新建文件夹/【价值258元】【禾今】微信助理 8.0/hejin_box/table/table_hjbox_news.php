...
class table_hjbox_news extends discuz_table{
    public function __construct() {
        $this->_table = 'hjbox_news';
        $this->_pk = 'id';
        parent::__construct();
    }


    public function fetch_fid_all($fid){
         return DB::fetch_all('select * from %t where fid = %d order by sort desc,add_time desc',array($this->_table,$fid));
    }
    public function fetch_fid_limit($fid,$startnum,$count){
         return DB::fetch_all('select * from %t where fid = %d order by sort desc,add_time desc limit %d,%d',array($this->_table,$fid,$startnum,$count));
    }
    ...
    public function fetch_tj_news($fid){
         return DB::fetch_first('select * from %t where istj = 1 and fid = %d  limit 0,1',array($this->_table,$fid));
    }
