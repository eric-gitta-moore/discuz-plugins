...
if(!file_exists($modfile)) {
	...
	cpmsg(lang('plugin/sanree_brand_video', 'error_install'), $brandplugurl, 'error');
...
$_G['cache']['plugin']['sanree_brand_video'] = $config;
$selectpriceunit = $config['selectpriceunit'];
$marr = explode("\r\n", $selectpriceunit);
foreach($marr as $row) {

	list($key , $val) = explode("=", $row);
	$config['selectpriceunitshow'][trim($key)] = video_shtmlspecialchars(trim($val));
