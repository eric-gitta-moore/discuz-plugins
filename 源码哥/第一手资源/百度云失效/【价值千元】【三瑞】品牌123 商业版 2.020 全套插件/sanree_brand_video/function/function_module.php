...
function video_fixdiscount($price, $minprice){
...
function video_add_diy_template($template, $arr){
...
function video_diystyle() {
	...
	$stamplist=C::t('#sanree_brand_video#sanree_brand_video_diystyle')->fetch_all_by_search($condition, $orderby);
	...
	diystylelog($style, "sanree_brand_video_diy.css");
	
}

function video_diytemplate($diytemplateid = 0) {
	...
	$stamplist=C::t('#sanree_brand_video#sanree_brand_video_diytemplate')->fetch_all_by_search($condition, $orderby);
	foreach($stamplist as $key => $val) {
	
	    $val['name'] = $val['issys']==1 ? srlang('neizhi').$val['name'] : $val['name'];
		$arr = array(
			'name' => $val['name'],
			'blockclass' => 'sanree_brand_video',
		);	
		$val['content'] = unserialize($val['content']) ? unserialize($val['content']) : $val['content'];
		$val['content'] = fixhtmlstr($val['content']);
		video_add_diy_template($val['content'], $arr);
		
	}		
	
}

function video_srconvertunusedattach($aid, $tid, $pid, $uid, $description) {
	global $_G;
	if(!$aid) {
		return;
	}
...
function video_setattachment($bid, $caidstr, $tid, $pid, $homeaid, $uid) {
	...
	foreach($aids as $aid) {
		...
		video_srconvertunusedattach($aid, $tid, $pid, $uid, $_G['sr_attachnew'][$aid][description]);
		$inaddstr.= '[attach]'.$aid.'[/attach]'."\r\n";
		
	}
	$tidata = C::t('#sanree_brand#forum_post')->fetch_threadpost_by_tid_invisible($tid);
	$message = $tidata['message'];
	$message = preg_replace('/\[attachvideo\]/is', $inaddstr, $message);
...
function video_chkmodeend($result,$isshow = TRUE) {
...
function video_gethome($video) {
	...
	$tid = $_G['tid'] = intval($video['tid']);
	$attachs =  getattach($video[pid]);
	if ($attachs['attachs']['used']) {
		foreach($attachs['attachs']['used'] as $key => $value) {
			if ($value['aid'] == $video[homeaid]) {
			
				return video_getforumimg($value['aid'], 1, 300, 300, 'fixnone').'&ramdom='.random(5);
				
			}
		}
		return video_getforumimg($attachs['attachs']['used'][0]['aid'], 1, 300, 300, 'fixnone').'&ramdom='.random(5);
	}
	return 'static/image/common/nophoto.gif';
}
function video_dsign($str, $length = 16){
	return substr(md5(getglobal('uid').$str.getglobal('authkey')), 0, ($length ? max(8, $length) : 16));
}

function video_getforumimg($aid, $nocache = 0, $w = 140, $h = 140, $type = '') {
	global $_G;
	$key = video_dsign($aid.'|'.$w.'|'.$h);
	return 'plugin.php?id=sanree_brand_video&mod=image&aid='.$aid.'&size='.$w.'x'.$h.'&key='.rawurlencode($key).($nocache ? '&nocache=yes' : '').($type ? '&type='.$type : '');
}

function video_fixthreadpic($aid, $ispic = 0){
	$pic = video_getforumimg($aid, 0, 600, 0, 'fixnone');
...
function video_shtmlspecialchars($string){
	return str_replace(array('&amp;', '&quot;', '&lt;', '&gt;'), array('&', '"', '<', '>') , $string);
}

function video_brandgetburl($bid){
	global $_G;
...
function video_showsetting($title, $id, $allkey, $allvalue, $type){
    global $lang;
if ($type == 'mcheckbox') {
