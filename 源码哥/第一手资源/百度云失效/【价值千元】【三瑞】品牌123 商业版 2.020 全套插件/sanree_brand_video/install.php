...
$sql = <<<EOF
CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video_category` (
...
INSERT INTO `pre_sanree_brand_video_category` (`cateid`, `keywords`, `description`, `name`, `status`, `dateline`, `displayorder`) VALUES
...
CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video` (
...
CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video_voter` (
...
CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video_voterlog` (
  `tid` mediumint(8) unsigned NOT NULL default '0',
  `bid` mediumint(8) NOT NULL default '0',
  `gid` mediumint(8) NOT NULL,
  `uid` mediumint(8) unsigned NOT NULL default '0',
...
CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video_diystyle` (
...
INSERT INTO `pre_sanree_brand_video_diystyle` (`diystyleid`, `name`, `content`, `uid`, `username`, `dateline`, `displayorder`, `status`, `issys`) VALUES
(1, 'hotvideo', 's:62:".stylename{\\r\\n}\\r\\n.stylename ul li{\\r\\n     line-height:20px;\\r\\n}\\r\\n";', 1, 'admin', 1350793879, 0, 1, 1);

CREATE TABLE IF NOT EXISTS `pre_sanree_brand_video_diytemplate` (
...
INSERT INTO `pre_sanree_brand_video_diytemplate` (`diytemplateid`, `name`, `content`, `uid`, `username`, `dateline`, `displayorder`, `status`, `issys`) VALUES
...
if (!exitscolumn('isvideo','sanree_brand_businesses_module')) {
	$sql="ALTER TABLE `pre_sanree_brand_businesses_module` ADD `isvideo` TINYINT( 1 ) NULL DEFAULT '0';";
	runquery($sql);	
	$sql = "INSERT INTO `pre_sanree_brand_language` (`module` ,`langkey` ,`langvalue` ) VALUES ('video', 'isvideo', '".$installlang[isvideo]."');";
	runquery($sql);		
}
if (!exitscolumn('maxvideo','sanree_brand_group_module')) {
	$sql="ALTER TABLE `pre_sanree_brand_group_module` ADD `maxvideo` INT( 11 ) NULL DEFAULT '20';";
	runquery($sql);	
	$sql = "INSERT INTO `pre_sanree_brand_language` (`module` ,`langkey` ,`langvalue` ) VALUES ('video', 'maxvideo', '".$installlang[maxvideo]."');";
	runquery($sql);	
}
