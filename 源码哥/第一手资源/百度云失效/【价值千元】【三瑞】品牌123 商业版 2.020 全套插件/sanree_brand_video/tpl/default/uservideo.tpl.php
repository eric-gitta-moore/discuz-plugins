...
<link rel="stylesheet" type="text/css" href="{sr_brand_video_TPL}uservideo.css?{VERHASH}" />
    ...
    <div class="albumlist">
	  <div class="albumleft">
	    ...
	    <div class="videolist">
		   <div class="ahd">
		   <div class="ubar">
			   <!--{if defined('IN_BRAND_USER')}--><a href="plugin.php?id=sanree_brand_video&mod=published&amp;bid={$brandresult[bid]}" onclick="showWindow('publisheddlg', this.href, 'get', 1)" class="y creatalbum"></a><!--{/if}-->	   
		   ...
		   <h1><a href="{$brandresult[url]}">{$brandresult[name]}</a> <span>&gt; </span>{lang sanree_brand_video:mode_video}</h1></div>
		   <div class="abd">
		         <!--{if $videolist}-->
					 <ul class="videodata">
					 <!--{loop $videolist $video}-->
					 <li><div class="videoimg"><a href="{$video[url]}" onclick="atarget(this)"><img src="{$video[pic]}" /></a></div>
						  <h1><a href="{$video[url]}" title="{$video[name]}" onclick="atarget(this)">{$video[name]}</a></h1>
						  </li>
					 <!--{/loop}-->
					 </ul>	
					 <div class="pager">{$multi}</div>
				 <!--{else}-->
				 <div class="zanwu">{lang sanree_brand_video:novideo}</div>
				 <!--{/if}-->
		   </div>
		</div> 	 
