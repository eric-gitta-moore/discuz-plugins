 ...
 *
 *      $Id: sanree_brand_video_diyconfig.php $
 */
...
if ($do == 'fixbranddiy') {
	...
	$entrydir = DISCUZ_ROOT.'source/plugin/sanree_brand_video/block/sanree';
	...
	$target=$blocksanreedir ."/block_brand_video.php";
	if(file_exists($target)){
	  @unlink($target);
	}			
	if(!file_exists($target)) {
		$source=$entrydir."/block_brand_video.php";
		@copy($source, $target);
	}
	video_diytemplate();	
	video_diystyle();	
	cpmsg($langs['blockstyle_create_succeed'], 'action=plugins&operation=config&identifier=sanree_brand_video&pmod=admincp&act=diyconfig', 'succeed');	
...
elseif ($do == 'list') {
		...
		array(
			'<ul class="tab1">'.
			'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diyconfig&identifier=sanree_brand_video&pmod=admincp"><span>'.$langs['diyconfig'].'</span></a></li>'.
	...
	<script language="javascript">disallowfloat = 'newthread';</script>
		...
		function fixbranddiy(){
			...
			{
				location.href= '<?php echo ADMINSCRIPT?>?action=plugins&operation=config&act=diyconfig&do=fixbranddiy&identifier=sanree_brand_video&pmod=admincp';
			}
		}		
		</script><tr>
