 ...
 *
 *      $Id: class_sanree_brand_video_category.php sanree $
 */

if(!defined('IN_DISCUZ')) {
	exit('Access Denied');
}
class sanree_brand_video_category{
	...
	function __construct($identifier){
		...
		$this->sanree_brand_video_category($identifier);
		
	}
	
	function sanree_brand_video_category($identifier){
	...
	function show($is_rewrite){
		...
		$this->_category_list[0] = array('index' => 0,'cateid'=>0,'class' => $act,'name' => video_modlang('all'), 'url' => $modelurl);
		$this->_location = srlang('all');
		if (!is_array($_G['cache']['sanree_brand_video_category'])) {
			video_updatecache('category');
		}
		$this->_subcategory = video_loadcache('subcategory');
		$this->_category = video_loadcache('category');
		$this->_navigation = '';
		...
		foreach($this->_subcategory as $value) {
			...
			$value['url'] = video_getcateurl(array('tid' => $value['cateid']));
			...
			foreach ($value['subcategories'] as $subkey =>$subcate) {
				...
				$subarr[$subkey]['url'] = video_getcateurl(array('tid' => $subcate['cateid']));
		...
		if ($cateid>0) {
		    $this->_nowcate['name'] = video_modlang('all');
			$this->_pid = intval($this->_category[$cateid]['pcateid']);
			if ($this->_pid ==0) {
				$this->_pid = $cateid;
				$this->_nowcate['class'] = ' class="a"';
				$this->_nowcate['url'] = video_getcateurl(array('tid' => $cateid));
				$this->_curcatedata = $this->_subcategory[$cateid];
			}
			else {
				$purl = video_getcateurl(array('tid' => $this->_pid));
			...
			foreach($this->_subcategory[$this->_pid]['subcategories'] as $value) {
			    ...
			    $count = C::t('#sanree_brand_video#sanree_brand_video')->count_by_cateid($value['cateid']);
				$value['url'] = video_getcateurl(array('tid' => $value[cateid]));
				if ($value['cateid'] == $cateid) {
	...
	function getcatenavigation($cateid, &$navigation, $addstr='', $url='') {
		...
		if ($value){
			$purl= video_getcateurl(array('tid' => $value[cateid]));
