 ...
 *
 *      $Id: class_sanree_brand_maps_category.php sanree $
 */
...
class sanree_brand_category{
	...
	function show($is_rewrite){
		...
		$this->_category_list[0] = array('index' => 0,'cateid'=>0,'class' => $act,'name' => srlang('all'), 'url' => $modelurl);
		...
		foreach($this->_subcategory as $value) {
			...
			$value['url'] = maps_getcateurl(array('tid' => $value[cateid]));
			...
			foreach ($value['subcategories'] as $subkey =>$subcate) {
				...
				$subarr[$subkey][url] = maps_getcateurl(array('tid' => $subcate[cateid]));
		...
		if ($cateid>0) {
			...
			if ($this->_pid ==0) {
				...
				$purl = maps_getcateurl(array('tid' => $this->_pid));
	...
	function getcatenavigation($cateid, &$navigation, $addstr='', $url='') {
		...
		if ($value){
			$purl= maps_getcateurl(array('tid' => $value[cateid]));
