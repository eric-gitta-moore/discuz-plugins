...
<link rel="stylesheet" type="text/css" href="{sr_brand_coupon_TPL}usercoupon.css?{VERHASH}" />
    ...
    <div class="albumlist">
	  <div class="albumleft">
	    ...
	    <div class="couponlist">
		   <div class="ahd">
		   <div class="ubar">
			   <!--{if defined('IN_BRAND_USER')}--><a href="plugin.php?id=sanree_brand_coupon&mod=published&amp;bid={$brandresult[bid]}" onclick="showWindow('publisheddlg', this.href, 'get', 1)" class="y creatalbum"></a><!--{/if}-->	   
		   ...
		   <h1><a href="{$brandresult[url]}">{$brandresult[name]}</a> <span>&gt; </span>{lang sanree_brand_coupon:mode_coupon}</h1></div>
		   <div class="abd">
		         <!--{if $couponlist}-->
					 <ul class="coupondata">
					 <!--{loop $couponlist $coupon}-->
					 <li><div class="couponimg"><a href="{$coupon[url]}" onclick="atarget(this)"><img src="{$coupon[pic]}" /></a></div>
						  <div class="cnames"><a href="{$coupon[url]}" title="{$coupon[name]}" onclick="atarget(this)">{$coupon[name]}</a></div>
						  <div class="price"><span>{$config['selectpriceunitshow'][$coupon[priceunit]]}{$coupon[price]}</span></div>
						  <div class="minprice"><span>{$config['selectpriceunitshow'][$coupon[priceunit]]}{$coupon[minprice]}</span></div>
						  </li>
					 <!--{/loop}-->
					 </ul>	
