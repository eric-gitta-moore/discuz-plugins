...
$newcount = C::t('#sanree_brand_coupon#sanree_brand_coupon')->count_by_where(' AND status<>1');
$langs = $scriptlang['sanree_brand_'.$pidentifier];
$rightlink='<div style="float:right;font-size:12px;">';
$rightlink.='<a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act='.$pidentifier.'&do=couponaudit&identifier=sanree_brand_'.$pidentifier.'&pmod=admincp">'.$langs[$pidentifier.'new'].'('.$newcount.')</a>';
...
$rightlink.='|<a href="plugin.php?id=sanree_brand_'.$pidentifier.'" target="_blank">'.$langs['manage_nav_pluginshow_'.$pidentifier].'</a>';
