 ...
 *
 *      $Id: sanree_brand_coupon_fastpost.php sanree $
 */
...
$cid = intval($couponresult[cid]);
$tid = intval($couponresult[tid]);
$bid = intval($couponresult[bid]);
...
if(submitcheck('postsubmit', 0 , $seccodecheck , $secqaacheck)) {
	...
	if ($couponresult['allowreply']!=1) {
	
		showmessage(coupon_modlang('nofastpostcontrol'));
	...
	$_POST['portal_referer'] = coupon_getitemurl($cid);
	$_G['tid'] = $tid;
	$inspacecpshare = TRUE;
	require_once libfile('function/forum');
	require libfile('forum/post', 'module') ;
	C::t('#sanree_brand_coupon#sanree_brand_coupon_voter')->updatestar_by_tid($_G['uid'], $_G['username'], $tid, intval($_G['sr_star']));
	...
	if ($couponresult['allowreply']!=1) {
		...
		ajaxexit(coupon_modlang('nofastpostcontrol'));
	...
	if ($count>0) {
		...
		$murl= 'plugin.php?id=sanree_brand_coupon&mod=fastpost&tid='.$tid.$extra;
