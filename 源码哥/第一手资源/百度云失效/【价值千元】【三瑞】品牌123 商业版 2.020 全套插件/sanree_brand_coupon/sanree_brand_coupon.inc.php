 ...
 *
 *      $Id: sanree_brand_coupon.inc.php sanree $
 */
///error_reporting(E_ALL);
if(!defined('IN_DISCUZ')) {
	exit('Access Denied');
}
$_G['pidentifier'] = $pidentifier = 'coupon';
if (CHARSET=='utf-8') {
...
if(!file_exists($modfile)) {
	...
	showmessage(lang('plugin/sanree_brand_coupon', 'error_install'), $brandplugurl, 'error');
...
$pluginversion = $_G['setting']['plugins']['version']['sanree_brand_coupon'];
$is_rewrite = $config['is_rewrite'];
$discuz_version = $appVer; 
$modarray = array('index', 'my'.$pidentifier, 'published', $pidentifier.'show', 'user'.$pidentifier, 'fastpost','delete'.$pidentifier, 'image', 'print','watermask','members');
...
$watermarkfont = $config['watermarkfont'];
...
$rebateunit = intval($config['rebateunit']);
$rebateunitname = $_G['setting']['extcredits'][$rebateunit]['title'];
$consumeproportion = intval($config['consumeproportion']);
...
if ($rebateunit<1) {
	...
	showmessage(coupon_modlang('nocreditunit'));
...
if ($is_rewrite==1) {
	$_G['basefilename'] = $_G['basescript'] = trim($config['urlcouponmode']);
} else {
