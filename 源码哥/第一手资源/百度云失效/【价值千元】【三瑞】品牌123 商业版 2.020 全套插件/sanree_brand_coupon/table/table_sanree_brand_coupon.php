 ...
 *
 *      $Id: table_sanree_brand_coupon.php $
 */
...
class table_sanree_brand_coupon  extends discuz_table {
	...
	public function __construct() {
		...
		$this->_table = 'sanree_brand_coupon';
		$this->_jointable = 'sanree_brand_coupon_category';
	...
	public function getcoupon_by_cid($cid) {
	...
	public function usergetcoupon_by_cid($cid, $uid) {
	...
	public function getcoupon_by_tid($tid) {
		return XDB::fetch_first("SELECT t.*,c.name AS catename,m.username AS user,b.name as brandname,tt.views as viewnum,b.tel,b.address,b.qq,b.allowmultiple FROM %t as t 
		LEFT JOIN %t as c on t.cateid=c.cateid 
		LEFT JOIN %t m ON m.uid=t.uid 
		LEFT JOIN %t b ON b.bid=t.bid
		LEFT JOIN %t tt ON t.tid=tt.tid
