...
function news_add_diy_template($template, $arr){
...
function news_diystyle() {
	...
	$stamplist=C::t('#sanree_brand_news#sanree_brand_news_diystyle')->fetch_all_by_search($condition, $orderby);
	...
	diystylelog($style, "sanree_brand_news_diy.css");
	
}

function news_diytemplate($diytemplateid = 0) {
	...
	$stamplist=C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->fetch_all_by_search($condition, $orderby);
	foreach($stamplist as $key => $val) {
	
	    $val['name'] = $val['issys']==1 ? srlang('neizhi').$val['name'] : $val['name'];
		$arr = array(
			'name' => $val['name'],
			'blockclass' => 'sanree_brand_news',
		);	
		$val['content'] = unserialize($val['content']) ? unserialize($val['content']) : $val['content'];
		$val['content'] = fixhtmlstr($val['content']);
		news_add_diy_template($val['content'], $arr);
		
	}		
	
}

function news_srconvertunusedattach($aid, $tid, $pid, $uid, $description) {
	global $_G;
	if(!$aid) {
		return;
	}
...
function news_setattachment($bid, $caidstr, $tid, $pid, $homeaid, $uid) {
	...
	foreach($aids as $aid) {
		...
		news_srconvertunusedattach($aid, $tid, $pid, $uid, $_G['sr_attachnew'][$aid][description]);
...
function news_chkmodeend($result,$isshow = TRUE) {
...
function news_gethome($news) {
	...
	$tid = $_G['tid'] = intval($news['tid']);
	$attachs =  getattach($news[pid]);
	if ($attachs['attachs']['used']) {
		foreach($attachs['attachs']['used'] as $key => $value) {
			if ($value['aid'] == $news[homeaid]) {
			
				return news_getforumimg($value['aid'], 1, 300, 300, 'fixnone').'&ramdom='.random(5);
				
			}
		}
		return news_getforumimg($attachs['attachs']['used'][0]['aid'], 1, 300, 300, 'fixnone').'&ramdom='.random(5);
	}
	return 'static/image/common/nophoto.gif';
}
function news_dsign($str, $length = 16){
	return substr(md5(getglobal('uid').$str.getglobal('authkey')), 0, ($length ? max(8, $length) : 16));
}

function news_getforumimg($aid, $nocache = 0, $w = 140, $h = 140, $type = '') {
	global $_G;
	$key = news_dsign($aid.'|'.$w.'|'.$h);
	return 'plugin.php?id=sanree_brand_news&mod=image&aid='.$aid.'&size='.$w.'x'.$h.'&key='.rawurlencode($key).($nocache ? '&nocache=yes' : '').($type ? '&type='.$type : '');
}

function news_fixthreadpic($aid, $ispic = 0){
	$pic = news_getforumimg($aid, 0, 600, 0, 'fixnone');
...
function news_shtmlspecialchars($string){
	return str_replace(array('&amp;', '&quot;', '&lt;', '&gt;'), array('&', '"', '<', '>') , $string);
}

function news_brandgetburl($bid){
	global $_G;
...
function news_srshowsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '', $nofaq = false) {
	...
	if($type == 'srimg') {
		if ($_G['setting']['version']=='X2') {
			...
			define(IMGDIR, 'source/plugin/sanree_brand_news/X2/images/');
			$thisconfig= array();
		...
		$thisconfig[realjspath] = 'source/plugin/sanree_brand_news/X2/js/';
