 ...
 *
 *      $Id: sanree_brand_news.inc.php sanree $
 */
///error_reporting(E_ALL);
if(!defined('IN_DISCUZ')) {
	exit('Access Denied');
}
$_G['pidentifier'] = $pidentifier = 'news';
if (CHARSET=='utf-8') {
...
if(!file_exists($modfile)) {
	...
	showmessage(lang('plugin/sanree_brand_news', 'error_install'), $brandplugurl, 'error');
...
$pluginversion = $_G['setting']['plugins']['version']['sanree_brand_news'];
$is_rewrite = $config['is_rewrite'];
$discuz_version = $appVer; 
$modarray = array('index', 'my'.$pidentifier, 'published', 'attachlist', $pidentifier.'show', 'user'.$pidentifier, 'fastpost','delete'.$pidentifier, 'image');
...
if ($is_rewrite==1) {
	$_G['basefilename'] = $_G['basescript'] = trim($config['urlnewsmode']);
} else {
