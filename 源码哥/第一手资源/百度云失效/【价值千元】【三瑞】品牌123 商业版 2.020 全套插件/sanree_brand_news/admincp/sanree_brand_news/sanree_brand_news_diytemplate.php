 ...
 *
 *      $Id: sanree_brand_news_diytemplate.php $
 */
...
if ($do == 'upgrading') {
	...
	if(submitcheck('addsubmit')){
		...
		if ($diytemplateid) {
			C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->srupdate($diytemplateid, $setarr, 'issys<>1');
		} else {

		    $setarr['dateline'] = TIMESTAMP;
			$setarr['uid'] = $_G[uid];
			$setarr['username'] = $_G[username];					
			C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->insert($setarr);
		...
		array(
			...
			'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand_news&pmod=admincp"><span>'.$langs['diytemplate'].'</span></a></li>'.
			'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand_news&pmod=admincp"><span>'.$langs['diystyle'].'</span></a></li>'.
			'</ul>'
		));
		showtablefooter();		
		if($diytemplateid>0) {
		    $menustr = $langs['edittemplate'];
		    $result = C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->get_by_diytemplateid($diytemplateid);	
        }
		else {	
	...
	if(submitcheck('submit')){
		...
		if(is_array($_G['sr_diytemplate_name'])) {
			foreach($_G['sr_diytemplate_name'] as $id => $title) {
				if(!$_G['sr_delete'][$id]) {
					...
					C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->srupdate($id,$setarr, 'issys<>1');
				}
			}
		}	
		...
		if($_G['sr_delete']) {
			...
			C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->srdelete($_G['sr_delete'], 'issys<>1');
	...
	{	
		...
		$count = C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->count_by_wherec($searchtext);
		...
		$datalist = C::t('#sanree_brand_news#sanree_brand_news_diytemplate')->fetch_all_by_searchc($searchtext,$orderby,(($page - 1) * $perpage),$perpage);
		...
		foreach($datalist as $value) {
			...
			$showedithtml = $value[issys]!=1 ? '<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand_news&pmod=admincp&do=upgrading&diytemplateid='.$value['diytemplateid']."&page=".$page.'\'">'.$lang['edit'].'</a>' : '';
			...
			'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand_news&pmod=admincp&do=copyrow&diytemplateid='.$value['diytemplateid']."&page=".$page.'\'">'.$langs['copynewtemplate'].'</a>',
			$showedithtml
			));
			
		}
		echo '<tr><td>&nbsp;</td><td colspan="7"><div><a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand_news&pmod=admincp&do=upgrading\'" class="addtr">'.$langs['addtemplate'].'</a></div></td></tr>';			
