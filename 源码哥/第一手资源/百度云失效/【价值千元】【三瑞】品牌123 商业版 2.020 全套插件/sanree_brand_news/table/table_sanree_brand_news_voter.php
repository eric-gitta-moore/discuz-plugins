 ...
 *
 *      $Id: table_sanree_brand_news_voter.php $
 */
...
class table_sanree_brand_news_voter  extends discuz_table {

	public function __construct() {

		$this->_table = 'sanree_brand_news_voter';
		$this->_pk    = 'gid';
	...
	public function updatestar_by_tid($uid, $username, $tid, $star = 1) {
		...
		$hcount = C::t('#sanree_brand_news#sanree_brand_news_voterlog')->getvoter_by_tid_uid($uid, $tid);
		if ($hcount>0){
			return 0;
		}
		$data = array(
		...
		C::t('#sanree_brand_news#sanree_brand_news_voterlog')->insert($data);
	...
	public function updatestar_by_gid($uid, $username, $gid, $star = 1) {
	    !in_array($star,array(1,2,3,4,5)) && $star = 1;
		$hcount = C::t('#sanree_brand_news#sanree_brand_news_voterlog')->getvoter_by_gid_uid($uid, $gid);
		if ($hcount>0){
			return 0;
		}
		$data = array(
