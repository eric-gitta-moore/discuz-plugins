...
<link rel="stylesheet" type="text/css" href="{sr_brand_news_TPL}usernews.css?{VERHASH}" />
    ...
    <div class="albumlist">
	  <div class="albumleft">
	    ...
	    <div class="newslist">
		   <div class="ahd">
		   <div class="ubar">
			   <!--{if defined('IN_BRAND_USER')}--><a href="plugin.php?id=sanree_brand_news&mod=published&amp;bid={$brandresult[bid]}" onclick="showWindow('publisheddlg', this.href, 'get', 1)" class="y creatalbum"></a><!--{/if}-->	   
		   ...
		   <h1><a href="{$brandresult[url]}">{$brandresult[name]}</a> <span>&gt; </span>{lang sanree_brand_news:mode_news}</h1></div>
		   <div class="abd">
		         <!--{if $newslist}-->
					 <ul class="newsdata">
					 <!--{loop $newslist $news}-->
					 ...
					 <div class="tiline">
					 <h1>
					 <a href="{$news[url]}" title="{$news[name]}" onclick="atarget(this)">{$news[name]}</a>&nbsp;<!--{if $news[homeaid]}--><img src="source/plugin/sanree_brand_news/tpl/default/images/img.jpg" /><!--{/if}--></h1>
					 <div class="tdateline">{$news[dateline]}</div>
					 </div>
					 <div class="inline"><div class="catenames">{lang sanree_brand_news:newscate}<span>{$news[catename]}</span></div> {lang sanree_brand_news:viewsh}<span>{$news[views]}</span></div>
						  </li>
					 <!--{/loop}-->
					 </ul>	
					 <div class="pager">{$multi}</div>
				 <!--{else}-->
				 <div class="zanwu">{lang sanree_brand_news:nonews}</div>
				 <!--{/if}-->
		   </div>
		</div> 	 
