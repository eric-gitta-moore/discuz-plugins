 ...
 *
 *      $Id: sanree_brand_guestbook_myguestbook_delete.php sanree $
 */
...
if(submitcheck('postsubmit')) {
	...
	if (!$isdeleteuser) {
		...
		showmessage(guestbook_modlang('error_delete'));
		
	}	
	C::t('#sanree_brand_guestbook#sanree_brand_guestbook')->delete($guestbookid);
    ///////C::t('#sanree_brand_guestbook#sanree_brand_guestbook')->update($guestbookid , array('isdelete' => -1, 'opdate' => TIMESTAMP, 'cpuid'=>$_G[uid]));
	$extra = array();
	...
	showmessage(guestbook_modlang('deletesucceed'), $url_forward, array(), $extra);	
