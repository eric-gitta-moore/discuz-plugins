...
if(!file_exists($modfile)) {
	...
	cpmsg(lang('plugin/sanree_brand_guestbook', 'error_install'), $brandplugurl, 'error');
...
$actlist =array(
	CURMODE_SANREE_GUESTBOOK	=> array(CURMODE_SANREE_GUESTBOOK),
);
$pdo = $navbar =array();
...
if(!in_array($act, $pdo)) {
	$act = CURMODE_SANREE_GUESTBOOK;
}
$current=' class="current"';
$menustr='<ul class="tab1">';
foreach ($actlist as $k => $v){
	$navbar[$k]=in_array($act,$v) ? $current:'';
	$menustr.='<li'.$navbar[$k].'><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act='.$v[0].'&identifier=sanree_brand_'.CURMODE_SANREE_GUESTBOOK.'&pmod=admincp"><span>'.$langs[$k].'</span></a></li>';
}
$menustr.='</ul>';
$thisurl = "plugins&operation=config&identifier=sanree_brand_".CURMODE_SANREE_GUESTBOOK."&pmod=admincp&act=$act";
$gotourl = 'action=plugins&operation=config&identifier=sanree_brand_'.CURMODE_SANREE_GUESTBOOK.'&pmod=admincp&act=';
$adminurl = ADMINSCRIPT.'?action=plugins&operation=config&identifier=sanree_brand_'.CURMODE_SANREE_GUESTBOOK.'&pmod=admincp';
...
if(!file_exists($actfile)) {
	...
	cpmsg($langs['isbusiness'], 'action=plugins&operation=config&identifier=sanree_brand_'.CURMODE_SANREE_GUESTBOOK.'&pmod=admincp&act=list', 'error');
