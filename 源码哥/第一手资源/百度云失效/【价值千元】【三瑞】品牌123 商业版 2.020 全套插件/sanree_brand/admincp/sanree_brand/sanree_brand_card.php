 ...
 *
 *      $Id: sanree_brand_card.php sanree $
 */
...
if ($do == 'ajax') {
		...
		foreach(C::t('#sanree_brand#sanree_brand_tag')->fetch_all_by_search($searchtext,$orderby,(($page - 1) * $perpage),$perpage) as $row) {
			...
			showtablerow('class="hover" style="cursor:pointer" onclick="settag(\''.$row['tagname'].'\')"', array(), array($row['tagname']));
	...
	if(submitcheck('addsubmit')) {
		...
		if($_G['sr_deletelogo']) {
			...
			if($_FILES['weixinpublicpic']) {
				...
				$data = array('extid' => $bid);
				$setarr['weixinpublicpic'] = upload_icon_banner($data, $_FILES['recommendimg'], 'brand_weixinpublicpic');
		...
		<?php	
		showsetting($langs['brandtag'], 'brandtag', $result['brandtag'], 'text','','','<button id="settag" onclick="showseltag();">'.$langs['selecttag'].'</button>&nbsp;&nbsp;Page:<input type="text" name="spage" size=2 id="spage" />', ' id="brandname" ');
	...
	if(submitcheck('submit')) {
		...
		if($view == 'cate') {
			...
			if($isselfdistrict) {
				...
				array(
					'<ul class="tab1">'.
					'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=card&view=cate&identifier=sanree_brand&pmod=admincp"><span>'.$langs['viewbycate'].'</span></a></li>'.
					'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=card&view=dis&identifier=sanree_brand&pmod=admincp"><span>'.$langs['viewbydis'].'</span></a></li>'.
					'</ul>'
				));
			}else {
				showtablerow('', array(), 
			...
			showsubtitle(array($langs['showorder'],  $langs['catename'],  $langs['status'], $langs['cardcount'], 'operation'));
			$category = C::t('#sanree_brand#sanree_brand_category')->getcategory_by_pcateid(0);
			foreach($category as $group) {
				$where = '&& cateid='.$group['cateid'].' && iscard=1';
				$count = C::t('#sanree_brand#sanree_brand_businesses')->count_by_where($where);
				...
				showtablerow('', array('class="td25"', 'class="td26"', 'class="td25"'), array(
							...
							$langs['yes'],
							$count,
							'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=card&identifier=sanree_brand&pmod=admincp&cateid='.$group[cateid].'\'">'.cplang('view').'</a>'
						));
			   ...
			   foreach($subcategory as $subgroup) {
				   ...
				   showtablerow('', array('class="td25"', 'class="td26"', 'class="td25"'), array(
								...
								"<div class=\"board\">$subgroup[name] $subststr</div>",
								$langs['yes'],
								$subcount,
								'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=card&identifier=sanree_brand&pmod=admincp&cateid='.$subgroup[cateid].'\'">'.cplang('view').'</a>'
							));  
			...
			array(
				'<ul class="tab1">'.
				'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=card&view=cate&identifier=sanree_brand&pmod=admincp"><span>'.$langs['viewbycate'].'</span></a></li>'.
				'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=card&view=dis&identifier=sanree_brand&pmod=admincp"><span>'.$langs['viewbydis'].'</span></a></li>'.
				'</ul>'
			));
			showtablefooter();
			...
			if($isselfdistrict==1) {
				...
				foreach($datalist as $row) {
					...
					$count,
					'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=card&identifier=sanree_brand&pmod=admincp&disname='.$row['name'].'\'">'.cplang('view').'</a>',
					in_array($level , array(0,1,2)) ? '<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=card&do=list&view=dis&identifier=sanree_brand&pmod=admincp&upid='.$row[id]."&page=".$_G['sr_page'].'\'" class="addtr">'.$langs['addsub'].'</a>' :''));
				...
				showtableheader($cardname, 'nobottom');
				...
				$searchtext = ' AND status=1 && iscard=1 ';	
				if (!empty($cateid) || !empty($disname)) {
				
					$searchtext .= "AND cateid ='".$cateid."' AND srbirthprovince = $disname OR srbirthcity = $disname OR srbirthdist = $disname OR srbirthcommunity = $disname";
			...
			if($disname) {
				$count = C::t('#sanree_brand#sanree_brand_businesses')->count_by_dis($searchtext);
			}else {		
				$count = C::t('#sanree_brand#sanree_brand_businesses')->count_by_where($searchtext);
			}
			if ($cateid) $extra .="&cateid=".$cateid;
			$multipage=multi($count, $perpage, $page, ADMINSCRIPT."?action=plugins&operation=config&act=$act&identifier=sanree_brand&pmod=admincp$extra");
			foreach(C::t('#sanree_brand#sanree_brand_businesses')->fetch_all_by_searchdis($searchtext,$disname,$orderby,(($page - 1) * $perpage),$perpage) as $row) {
