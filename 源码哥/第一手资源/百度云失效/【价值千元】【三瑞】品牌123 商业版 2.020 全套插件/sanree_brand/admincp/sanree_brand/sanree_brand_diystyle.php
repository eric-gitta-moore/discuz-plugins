...
if ($do == 'upgrading') {
	...
	if(submitcheck('addsubmit')){
		...
		array(
			...
			'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand&pmod=admincp"><span>'.$langs['diytemplate'].'</span></a></li>'.
			'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand&pmod=admincp"><span>'.$langs['diystyle'].'</span></a></li>'.
	...
	if(submitcheck('submit')){
		...
		if(is_array($_G['sr_diystyle_name'])) {
			foreach($_G['sr_diystyle_name'] as $id => $title) {
				if(!$_G['sr_delete'][$id]) {
					...
					C::t('#sanree_brand#sanree_brand_diystyle')->srupdate($id,$setarr, 'issys<>1');
				}
			}
		}	
		...
		if($_G['sr_delete']) {
			...
			C::t('#sanree_brand#sanree_brand_diystyle')->srdelete($_G['sr_delete'], 'issys<>1');
	...
	{	
		...
		$count = C::t('#sanree_brand#sanree_brand_diystyle')->count_by_wherec($searchtext);
		...
		$datalist = C::t('#sanree_brand#sanree_brand_diystyle')->fetch_all_by_searchc($searchtext,$orderby,(($page - 1) * $perpage),$perpage);
		...
		foreach($datalist as $value) {
			...
			'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand&pmod=admincp&do=copyrow&diystyleid='.$value['diystyleid']."&page=".$page.'\'">'.$langs['copynewstyle'].'</a>',			
			$value[issys]!=1 ? '<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand&pmod=admincp&do=upgrading&diystyleid='.$value['diystyleid']."&page=".$page.'\'">'.$lang['edit'].'</a>': ''
			));
			
		}
		echo '<tr><td>&nbsp;</td><td colspan="7"><div><a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand&pmod=admincp&do=upgrading\'" class="addtr">'.$langs['addstyle'].'</a></div></td></tr>';			
