...
class block_multibrand {

    var $setting = array();
	function name() {
		return lang('plugin/sanree_brand', 'blockclass_multibrandname');
	}
	...
	function fields() {
		return array(
				...
				'iscard' => array('name' => lang('plugin/sanree_brand', 'blockclass_brand_field_iscard'), 'formtype' => 'text', 'datatype' => 'pic'),
			);
	}
	...
	function getdata($style, $parameter) {
		...
		$wherearr[] = 't.isshow=1';
		$cateids = array();
		if(!empty($parameter['cateid'])) {
		
			if(isset($parameter['cateid'][0]) && $parameter['cateid'][0] == '0') {
			
				unset($parameter['cateid'][0]);
				
			}
			$cateids = $parameter['cateid'];
			
		}
		if (count($cateids)>0) {
			$wherearr[] = 't.cateid in ('.dimplode($cateids).')';
		}
		...
		foreach(C::t('#sanree_brand#sanree_brand_businesses')->fetch_all_forblock($where, $orderby, $ordersc, $itemsnum) as $value) {	
			...
			$datalist[] = array(
				...
				'fields' => array(
					...
					'iscard' => $value['iscard'] ? 'source/plugin/sanree_brand/tpl/good/images/cardico.jpg' : '',
				)
			);
