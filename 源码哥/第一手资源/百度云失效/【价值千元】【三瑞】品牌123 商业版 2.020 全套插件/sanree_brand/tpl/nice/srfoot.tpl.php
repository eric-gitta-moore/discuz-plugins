...
<div class="sr_comment">
	<div class="cbox">
			...
			<ul>
				...
				<li>
					...
					<div class="user_content r">
						<div class="uc">
							<div class="topmeta">
								<span class="l"><a href="home.php?mod=space&amp;uid={$thread['authorid']}" target=_blank>{$thread[author]}</a>
								</span>
								<span class="l">{lang sanree_brand:bird_item_report} <em><!--{$thread[dateline]}--></em></span> 
								<span class="l">{lang sanree_brand:satisfaction}</span>
								<div class="star">
									<span class="staroff"> <span class="staron" style="width: {$thread[satisfaction]}%;"></span></span>
								</div>
							</div>
							<div class="clear"></div>
		...
		<div class="report">
			<form autocomplete="off" id="fastpostform" name="fastpostform" action="plugin.php?id=sanree_brand&mod=fastpost" method="post" onSubmit="return fastnewpost(this);">
				...
				<div class="re_tit">
					{lang sanree_brand:bird_item_of} <strong>{$brandresult[name]}</strong> {lang sanree_brand:bird_item_idea}
				</div>
				<div class="input_area">
					...
					<div class="text_area r">
						<div class="t_infor">
							<div class="t_meta l">
								{lang sanree_brand:bird_item_hello}
								<a href="home.php?mod=space&amp;uid={$_G['uid']}">{$_G[username]}</a>
							</div>
							<div class="r_star r">
								<span class="l">{lang sanree_brand:bird_item_first}</span> 
								<!--{if !$satisfaction}-->
								<div class="star" id="newstar">
									<span onMouseover="setnewstar(1,'newstar',0)"><img src="source/plugin/sanree_brand/tpl/nice/images/staroff.png"  /></span>
									<span onMouseover="setnewstar(2,'newstar',0)"><img src="source/plugin/sanree_brand/tpl/nice/images/staroff.png"  /></span>
									<span onMouseover="setnewstar(3,'newstar',0)"><img src="source/plugin/sanree_brand/tpl/nice/images/staroff.png"  /></span>
									<span onMouseover="setnewstar(4,'newstar',0)"><img src="source/plugin/sanree_brand/tpl/nice/images/staroff.png" /></span>
									<span onMouseover="setnewstar(5,'newstar',0)"><img src="source/plugin/sanree_brand/tpl/nice/images/staroff.png" /></span>
								</div>
								<!--{else}-->
								<div class="star">
									<span class="staroff">
										<span class="staron" style="width: {$satisfaction}%;"></span>
									</span>
								</div>
								<!--{/if}--> 
							</div>
						...
						<div class="inputbox"> 
							...
							<!--{else}-->
							<div class="pt hm">
								{lang sanree_brand:login_to_reply}
								<a href="member.php?mod=logging&action=login" class="xi2">{lang login}</a> | 
								<a href="member.php?mod={$_G[setting][regname]}" class="xi2">$_G['setting']['reglinkname']</a>
							</div>
							<!--{/if}--> 
						</div>
						<div class="la"></div>
					...
					<!--{if $_G[uided]}--> 
						<!--{if $dzvflag}--> 
							<!--{if $seccodecheck}-->
							<div class="text_area r">
								{eval $sechash = !isset($sechash) ? 'S'.($_G['inajax'] ? 'A' : '').$_G['sid'] : $sechash.random(3); $sectpl = str_replace("'", "\'", $sectpl);}
								<!--{if $seccodecheck}--> 
						...
						<div class="text_area r">
							{eval $_G['sechashi'] = !empty($_G['cookie']['sechashi']) ? $_G['sechash'] + 1 : 0; $sechash = 'S'.($_G['inajax'] ? 'A' : '').$_G['sid'].$_G['sechashi']; $sectpl = !empty($sectpl) ? explode("<sec>", $sectpl) : array('<br />',': ','<br />','');$sectpldefault = $sectpl;$sectplqaa = str_replace('<hash>', 'qaa'.$sechash, $sectpldefault); $sectplcode = str_replace('<hash>', 'code'.$sechash, $sectpldefault); $secshow = !isset($secshow) ? 1 : $secshow; $sectabindex = !isset($sectabindex) ? 1 : $sectabindex;}
							<input name="sechash" type="hidden" value="$sechash" />
							<!--{if $sectpl}--> 
							<!--{if $secqaacheck}--> 
							{$sectplqaa[0]}{lang secqaa}{$sectplqaa[1]}
							<input name="secanswer" id="secqaaverify_$sechash" type="text" autocomplete="off" style="width:100px" class="txt px vm" onblur="checksec('qaa', '$sechash')" tabindex="$sectabindex" />
							<a href="javascript:;" onclick="updatesecqaa('$sechash');doane(event);" class="xi2">{lang seccode_change}</a>
							<span id="checksecqaaverify_$sechash">
								<img src="{STATICURL}image/common/none.gif" width="16" height="16" class="vm" />
							</span>
							$sectplqaa[2]
							<span id="secqaa_$sechash"></span> 
							<!--{if $secshow}-->
								<script type="text/javascript" reload="1">updatesecqaa('$sechash');</script><!--{/if}--> 
								$sectplqaa[3] 
							<!--{/if}--> 
							<!--{if $seccodecheck}--> 
								{$sectplcode[0]}{lang seccode}{$sectplcode[1]}
								<input name="seccodeverify" id="seccodeverify_$sechash" type="text" autocomplete="off" style="{if $_G[setting][seccodedata][type] != 1}ime-mode:disabled;{/if}width:100px" class="txt px vm" onblur="checksec('code', '$sechash')" tabindex="$sectabindex" />
								<a href="javascript:;" onclick="updateseccode('$sechash');doane(event);" class="xi2">{lang seccode_change}</a>
								<span id="checkseccodeverify_$sechash">
									<img src="{STATICURL}image/common/none.gif" width="16" height="16" class="vm" />
								</span>
								{$sectplcode[2]}
					...
					<!--{/if}-->
					<div class="c_btn">
						<a><input type="submit" value="{lang sanree_brand:bird_item_fourm}" align="absmiddle" /></a>
					</div>
				</div>
			</form>
		</div>
