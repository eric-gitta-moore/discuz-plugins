...
class table_forum_threadmod extends discuz_table
{
	...
	public function fetch_by_tid($tid) {
		return XDB::fetch_first('SELECT * FROM %t WHERE tid=%d ORDER BY dateline DESC LIMIT 1', array($this->_table, $tid));
	}
	public function fetch_by_tid_action_status($tid, $action, $status = 1) {
		return XDB::fetch_first('SELECT * FROM %t WHERE tid=%d AND action=%s AND status=%d ORDER BY dateline DESC LIMIT 1', array($this->_table, $tid, $action, $status));
	}
	...
	public function fetch_all_by_tid_magicid($tid, $magicid = 0) {
		return XDB::fetch_all('SELECT * FROM %t WHERE tid=%d AND magicid=%d', array($this->_table, $tid, $magicid));
	}
	public function fetch_all_by_tid($tid, $action = '', $start = 0, $limit = 0) {
		...
		return XDB::fetch_all("SELECT * FROM %t $wheresql ORDER BY dateline DESC ".XDB::limit($start, $limit), $parameter);
	}
	public function fetch_all_by_expiration_status($expiration, $status=1) {
		return XDB::fetch_all('SELECT * FROM %t WHERE expiration>0 AND expiration<%d AND status=%d', array($this->_table, $expiration, $status));
	}
	...
	public function delete_by_dateline($dateline) {
		...
		return XDB::delete($this->_table, XDB::field('tid', 0, '>').' AND '.XDB::field('dateline', $dateline, '<'));
	}
