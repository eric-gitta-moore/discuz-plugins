		...
		<tr>
			<td class="attswf">
				...
				<div id="attach_preview_$attach[aid]_menu"  style="display:none">
					<img src="{echo goods_getforumimg($attach[aid], 1, 300, 300, 'fixnone')}&ramdom={echo random(5)}" id="image_$attach[aid]" cwidth="{if $attach[width] < 300}$attach[width]{else}300{/if}"/>
				</div>
				<!--{/if}-->
			...
			<!--{if $_GET[result] != 'simple'}-->
				...
				<input type="hidden" name="attachnew[{$attach[aid]}][isimage]" value="<!--{if $attach['isimage']}-->1<!--{else}-->0<!--{/if}-->" />
				<!--{if $attach['isimage']}-->
				<input type="radio" name="ishome" class="px" value="$attach[aid]" size="1" <!--{if $attach[aid]==$homeaid}-->checked="checked" <!--{/if}--> />
				<!--{/if}-->
				</td>
			<!--{/if}-->
