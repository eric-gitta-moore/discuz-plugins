 ...
 *
 *      $Id: sanree_brand_goods_list.php 2 2012-01-03 13:05:56 sanree $
 */
...
if ($do == 'upgrading') {
	...
	if(submitcheck('addsubmit')){
		...
		if ($user) {
			...
			$t = C::t('#sanree_brand_goods#xcommon_member')->fetch_uid_by_username($user);	
			if($t) {
		...
		if ($cateid) {
			C::t('#sanree_brand_goods#sanree_brand_goods_category')->update($cateid, $setarr);
		}
		else {
		    $setarr['dateline'] = TIMESTAMP;
			C::t('#sanree_brand_goods#sanree_brand_goods_category')->insert($setarr);
		}
		...
		if($cateid>0) {
		    ...
		    $result = C::t('#sanree_brand_goods#sanree_brand_goods_category')->get_by_cateid($cateid);	
        }
		else {	
	...
	if(submitcheck('submit')){
		...
		if(is_array($newforum)) {
			foreach($newforum as $fup => $forums) {
				...
				$forum = C::t('#sanree_brand_goods#sanree_brand_goods_category')->fetch($fup);
				foreach($forums as $key => $forumname) {
					...
					C::t('#sanree_brand_goods#sanree_brand_goods_category')->insert($forumfields);
		...
		if(is_array($_G['sr_group_title'])) {	    
			foreach($_G['sr_group_title'] as $id => $title) {
				if(!$_G['sr_delete'][$id]) {
					...
					C::t('#sanree_brand_goods#sanree_brand_goods_category')->update($id,$setarr);
				}
			}
			
		}		
		if($_G['sr_delete']) {
			C::t('#sanree_brand_goods#sanree_brand_goods_category')->delete($_G['sr_delete']);
		}		
		goods_updatecache('category');
...
<?php
		...
		$category = C::t('#sanree_brand_goods#sanree_brand_goods_category')->getcategory_by_pcateid(0);
		foreach($category as $group) {
			$count = C::t('#sanree_brand_goods#sanree_brand_goods')->count_by_cateid($group['cateid']);
			...
			showtablerow('', array('', 'class="td25"', 'class="td26"'), array(
						...
						'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=list&do=upgrading&identifier=sanree_brand_goods&pmod=admincp&do=upgrading&cateid='.$group['cateid']."&page=".$page.'\'">'.$lang['edit'].'</a>',
						'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=goods&identifier=sanree_brand_goods&pmod=admincp&cateid='.$group['cateid'].'\'">'.cplang('view').'</a>'
					));
		   $subcategory = C::t('#sanree_brand_goods#sanree_brand_goods_category')->getcategory_by_pcateid($group['cateid']);
		   foreach($subcategory as $subgroup) {
		   
			   $subcount = C::t('#sanree_brand_goods#sanree_brand_goods')->count_by_cateid($subgroup['cateid']);
			   ...
			   showtablerow('', array('', 'class="td25"', 'class="td26"'), array(
							...
							'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=list&do=upgrading&identifier=sanree_brand_goods&pmod=admincp&do=upgrading&cateid='.$subgroup['cateid']."&page=".$page.'\'">'.$lang['edit'].'</a>',
							'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=goods&identifier=sanree_brand_goods&pmod=admincp&cateid='.$subgroup['cateid'].'\'">'.cplang('view').'</a>'
						));  
