 ...
 *
 *      $Id: sanree_brand_goods_diystyle.php $
 */
...
if ($do == 'upgrading') {
	...
	if(submitcheck('addsubmit')){
		...
		if ($diystyleid<1) {
			...
			$count=C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->count_by_where(" AND name='$name'");
			
		} else { 
		
			$count=C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->count_by_where(" AND name='$name' AND diystyleid <> '$diystyleid'");
		...
		if ($diystyleid) {
			C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->srupdate($diystyleid, $setarr, 'issys<>1');
		} else {

		    $setarr['dateline'] = TIMESTAMP;
			$setarr['uid'] = $_G[uid];
			$setarr['username'] = $_G[username];					
			C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->insert($setarr);
		...
		array(
			'<ul class="tab1">'.
			'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diyconfig&identifier=sanree_brand_goods&pmod=admincp"><span>'.$langs['diyconfig'].'</span></a></li>'.
			'<li><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diytemplate&identifier=sanree_brand_goods&pmod=admincp"><span>'.$langs['diytemplate'].'</span></a></li>'.
			'<li class="current"><a href="'.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand_goods&pmod=admincp"><span>'.$langs['diystyle'].'</span></a></li>'.
			'</ul>'
		));
		showtablefooter();		
		if($diystyleid>0) {
		    $menustr = $langs['editstyle'];
		    $result = C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->get_by_diystyleid($diystyleid);	
        }
		else {	
	...
	if(submitcheck('submit')){
		...
		if(is_array($_G['sr_diystyle_name'])) {
			foreach($_G['sr_diystyle_name'] as $id => $title) {
				if(!$_G['sr_delete'][$id]) {
					...
					C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->srupdate($id,$setarr, 'issys<>1');
				}
			}
		}	
		...
		if($_G['sr_delete']) {
			...
			C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->srdelete($_G['sr_delete'], 'issys<>1');
	...
	{	
		...
		$count = C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->count_by_wherec($searchtext);
		...
		$datalist = C::t('#sanree_brand_goods#sanree_brand_goods_diystyle')->fetch_all_by_searchc($searchtext,$orderby,(($page - 1) * $perpage),$perpage);
		...
		foreach($datalist as $value) {
			...
			'<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand_goods&pmod=admincp&do=copyrow&diystyleid='.$value['diystyleid']."&page=".$page.'\'">'.$langs['copynewstyle'].'</a>',			
			$value[issys]!=1 ? '<a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand_goods&pmod=admincp&do=upgrading&diystyleid='.$value['diystyleid']."&page=".$page.'\'">'.$lang['edit'].'</a>': ''
			));
			
		}
		echo '<tr><td>&nbsp;</td><td colspan="7"><div><a href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&act=diystyle&identifier=sanree_brand_goods&pmod=admincp&do=upgrading\'" class="addtr">'.$langs['addstyle'].'</a></div></td></tr>';			
