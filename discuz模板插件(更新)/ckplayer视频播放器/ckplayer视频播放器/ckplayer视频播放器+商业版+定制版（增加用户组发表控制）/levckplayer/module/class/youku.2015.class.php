...
class Youku {
	...
	public static function _getYouku($vid){
		$base = 'http://play.youku.com/play/get.json?ct=12&vid=';
		$base = 'http://play.youku.com/play/get.json?callback=jsonp3&ct=62&dv=pc&uid=0&_=1463764938657&vid=';
		$vid = str_ireplace('==', '==&', $vid);
		$link = $base.$vid;
		$retval = Base::_cget($link);
		if ($retval) {
			$rs = json_decode(substr(trim(strstr($retval, '{')), 0, -1), true);//print_r($rs);//return '';
			...
			$seed = $rs['data'][0]['seed'];
			$segs = $rs['data']['stream'][0]['segs'];//print_r($segs);
			$ip = $rs['data']['security']['ip'];
			$bsegs =  $brs['data']['stream'][0]['segs'];
			$_ep = $rs['data']['security']['encrypt_string'];
			$fileId = $rs['data']['stream'][0]['stream_fileid'];
			list($sid, $token) = explode('_', self::yk_e('becaf9be', self::yk_na($_ep)));
			foreach ($segs as $key=>$val) {
				...
				$fileType = $typeArray['3gphd'];
				$data['high'][$key] = "http://k.youku.com/player/getFlvPath/sid/".$_ep."_00/st/{$fileType}/fileid/".$fileId."?K=".$_k."&hd=1&myp=0&ts=".((((($val['total_milliseconds_video'].'&ypp=0&ctype=12&ev=1&token=').$token).'&oip=').$ip).'&ep=').$ep;
				
			}
			//返回 图片 标题 链接  时长  视频
			$data['img'] = $rs['data'][0]['logo'];
			$data['title'] = $rs['data'][0]['title'];
			$data['seconds'] = $rs['data'][0]['seconds'];//print_r($data);
			return $data;
		} else {
			return false;
		}
