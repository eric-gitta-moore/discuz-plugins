...
class Iqiyi {
	...
	private static function getVrsEncodeCode($_arg1){
		...
		$_local5 = ($_local4 - 1);
		while ($_local5 >= 0){
			$_local6 = self::getVRSXORCode(intval($_local3[(($_local4 - $_local5) - 1)], 16), $_local5);
			$_local2 = (self::fromCharCode($_local6).$_local2);
			$_local5--;
		};
		return $_local2;
	}
	...
	private static function parseFlv($tvid,$vid,$type){
		...
		$video_datas = json_decode(self::_cget($api_url),true);
		...
		$time_datas = json_decode(self::_cget($time_url),true);
		...
		foreach($vs as $val){
			foreach ($val['fs'] as $v){
				...
				if($val['bid'] ==  4 || $val['bid'] ==  5 || $val['bid'] ==  10){
					$this_link = self::getVrsEncodeCode($this_link);
				}

				$sp = explode('/',$this_link);
				$files = explode('.',$sp[count($sp)-1]);
				$fileId = $files[0];
				$this_key = self::calmd($server_time,$fileId);
