...
class Qq {
    ...
    private static function getVideoUrl($vid){
        ...
        if($vi['st'] ==2 && $vi['fst'] == 5 && $infos['s'] == 'o'){
            ...
            //视频清晰度判定和视频排序
            if(!empty($normal)) ksort($normal); $data['normal']  = array_merge($normal);
            if(!empty($original)) ksort($original); $data['original']  = array_merge($original);
            if(!empty($high)) ksort($high); $data['high']  = array_merge($high);
            if(!empty($super)) ksort($super); $data['super']  = array_merge($super);
            $data['title'] = $vi['ti'];//标题
