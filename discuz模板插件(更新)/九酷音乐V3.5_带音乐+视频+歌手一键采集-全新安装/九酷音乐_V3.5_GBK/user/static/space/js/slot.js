var slotLib = {
	Init: function() {
		$('#button').click(function() {
			...
			$.getJSON(zone_domain + "index.php?p=account&a=doMoneyAdd&callback=?",
			function(data) {
				if (data['error'] == 20001) {
					$.tipMessage("您未登录，无法执行此操作。", 1, 3000, 0,
					function() {
						location.href = location.href;
					});
				} else if (data['error'] == 10002) {
					$.tipMessage("您已摇过一次，请中午12点以后再来。", 1, 3000, 0,
					function() {
						location.href = location.href;
					});
				} else if (data['error'] == 10003) {
					$.tipMessage("您今天摇奖资格已经用完，请明天再来。", 1, 3000, 0,
					function() {
						location.href = location.href;
					});
				} else {
