...
<head>
	...
	<title>待审视频 - <?php echo cd_webname; ?></title>
			...
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						...
						<li class="find">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=video&a=pass'); ?>">待审视频</a>
						</li>
					</ul>
					<div class="action">
				...
				<div class="main_nav2">
					<ul>
						<li<?php if($classId == ""){echo " class=\"current\"";} ?>><a href="<?php echo cd_upath.rewrite_url('index.php?p=video&a=pass'); ?>">全部类型</a></li>
						<?php
							global $db;
        						$query = $db->query("select CD_ID,CD_Name from ".tname('videoclass')." where CD_IsIndex=0 order by CD_TheOrder asc LIMIT 0,8");
        						while ($row = $db->fetch_array($query)) {
								echo "<li";
								if($classId == $row["CD_ID"]){
									echo " class=\"current\"";
								}
								echo "><a href=\"".cd_upath.rewrite_url("index.php?p=video&a=pass&classId=".$row["CD_ID"])."\">".$row["CD_Name"]."</a></li>";
							}
						?>
					</ul>
				...
				<div id="imageList" class="minHeight500">
						...
						<?php
							global $db;
							if($classId){
								$sql="select * from ".tname('video')." where CD_IsIndex=1 and CD_User='$qianwei_in_username' and CD_ClassID='$classId' order by CD_AddTime desc";
							}else{
								$sql="select * from ".tname('video')." where CD_IsIndex=1 and CD_User='$qianwei_in_username' order by CD_AddTime desc";
							}
							$Arr=getuserpage($sql,20);//sql,每页显示条数
							$result=$db->query($Arr[2]);
							$num=$db->num_rows($result);
							if($num==0){
								echo '<div class="nothing">暂无待审视频。</div>';
							}else{
								if($result){
									...
									while ($row = $db ->fetch_array($result)){
										echo '<li>';
										echo '<div class="pic">';
										echo '<a href="'.LinkUrl("video",$row['CD_ClassID'],1,$row['CD_ID']).'" target="_blank"><img class="avatar" src="'.LinkPicUrl($row['CD_Pic']).'" title="'.$row['CD_Name'].'" height="80" width="80" onerror="this.src=\''.cd_upath.'static/images/nopic.jpg\'"/></a>';
										echo '</div>';
...
<script type="text/javascript">listenMsg.start();nav.init();nav.userMenu();videoLib.passDelInit();</script>
</body>
</html>