...
<head>
	...
	<title>已审视频 - <?php echo cd_webname; ?></title>
		...
		<div class="user_main">
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						<li class="concert">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=video&a=list'); ?>">已审视频</a>
						</li>
						<li class="find">
				...
				<div class="main_nav2">
					<ul>
						<li<?php if($classId == ""){echo " class=\"current\"";} ?>><a href="<?php echo cd_upath.rewrite_url('index.php?p=video&a=list'); ?>">全部类型</a></li>
						<?php
							global $db;
								...
								echo "><a href=\"".cd_upath.rewrite_url("index.php?p=video&a=list&classId=".$row["CD_ID"])."\">".$row["CD_Name"]."</a></li>";
							}
						?>
					</ul>
				...
				<div id="imageList" class="minHeight500">
						...
						<?php
							global $db;
							if($classId){
								$sql="select * from ".tname('video')." where CD_IsIndex=0 and CD_User='$qianwei_in_username' and CD_ClassID='$classId' order by CD_AddTime desc";
							}else{
								$sql="select * from ".tname('video')." where CD_IsIndex=0 and CD_User='$qianwei_in_username' order by CD_AddTime desc";
							}
							$Arr=getuserpage($sql,20);//sql,每页显示条数
							$result=$db->query($Arr[2]);
							$num=$db->num_rows($result);
							if($num==0){
								echo '<div class="nothing">暂无已审视频。</div>';
							}else{
								if($result){
									...
									while ($row = $db ->fetch_array($result)){
										...
										echo '<label style="cursor:pointer;">'.getlen("len","10",$row['CD_Name']).'</label>';
										echo '</div>';
										echo '</li>';
									}
									echo '</ul>';
