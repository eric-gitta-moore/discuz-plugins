...
<head>
	...
	<title>照片排序 - <?php echo cd_webname; ?></title>
					...
					<div class="imageSort">
						<?php
							...
							if($num==0){
								...
								if($result){
									echo '<div id="imageSort" class="sortable">';
									while ($row = $db ->fetch_array($result)){
										$a=$a+1;
										echo '<img id="'.$row['cd_id'].'" class="avatar" src="'.getalbumthumb($row['cd_url'],1).'" uid="'.$row['cd_uid'].'" height="80" width="80" onerror="$call(function(){libs.imageError(this);}, this)"/>';
									}
									echo '</div>';
									echo '<div class="sort_button">';
...
<script type="text/javascript">nav.helpNoticeInit();albumLib.imageSortInit();</script>
</body>
</html>