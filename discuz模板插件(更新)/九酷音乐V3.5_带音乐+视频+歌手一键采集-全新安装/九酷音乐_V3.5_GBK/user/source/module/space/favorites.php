...
<head>
	...
	<title><?php echo $qianwei_web_nicheng; ?>收藏的音乐</title>
					...
					<div class="spaceMTitle2">
						...
						<ul>
							<li<?php if($classId == "0"){echo " class=\"current\"";} ?>>
								<a href="<?php echo cd_upath.rewrite_url('index.php?p=space&a=favorites&uid='.$qianwei_web_userid); ?>">全部类型</a>
								<b></b>
							</li>
							<?php
								global $db;
									...
									echo "><a href=\"".cd_upath.rewrite_url('index.php?p=space&a=favorites&uid='.$qianwei_web_userid.'&classId='.$row['CD_ID'])."\">".$row["CD_Name"]."</a><b></b></li>";
								}
							?>
							<?php if($qianwei_web_userid == $qianwei_in_userid){ ?>
							<li class="management">
								<a href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=favorites&i=me'); ?>">管理</a>
							</li>
							<?php } ?>
						</ul>
					</div>
						...
						<!--有内容-->
							<form id="list">
									<?php
										...
										$i=0;
										if($classId){
											$sql="select * from ".tname('fav')." where cd_uid='$qianwei_web_userid' and cd_classid='$classId' order by cd_addtime desc";
										}else{
											$sql="select * from ".tname('fav')." where cd_uid='$qianwei_web_userid' order by cd_addtime desc";
										}
										...
										if($num==0){
											if($classId){
												...
												echo '<div class="nothing_recommend">'.ReplaceStr($qianwei_web_callname,"我","").'还没有收藏任何音乐。</div>';
											}
										}else{
											if($result){
												...
												while ($row = $db ->fetch_array($result)){
													$i=$i+1;
													$music = $db->getrow("select CD_ID,CD_Name,CD_ClassID,CD_UserID,CD_User,CD_UserNicheng,CD_Points from ".tname('music')." where CD_ID='".$row['cd_musicid']."'");
													if($music){
														echo '<li';
														if($i%2==0){
															echo ' class="c2"';
