...
<head>
	...
	<title><?php echo $qianwei_web_nicheng; ?>上传的音乐</title>
						...
						<div class="spaceMTitle2">
							...
							<ul>
								<li<?php if($classId == "0"){echo " class=\"current\"";} ?>>
									<a href="<?php echo cd_upath.rewrite_url('index.php?p=space&a=dance&uid='.$qianwei_web_userid); ?>">全部类型</a>
									<b></b>
								</li>
								<?php
									global $db;
        								$query = $db->query("select * from ".tname('class')." where CD_SystemID=1 and CD_FatherID=1 and CD_IsHide=0 order by CD_TheOrder asc LIMIT 0,4");
        								while ($row = $db->fetch_array($query)) {
										echo "<li";
										if($classId == $row["CD_ID"]){
											echo " class=\"current\"";
										}
										echo "><a href=\"".cd_upath.rewrite_url('index.php?p=space&a=dance&uid='.$qianwei_web_userid.'&classId='.$row['CD_ID'])."\">".$row["CD_Name"]."</a><b></b></li>";
									}
								?>
								<?php if($qianwei_web_userid == $qianwei_in_userid){ ?>
								<li class="management">
									<a href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=share&i=me'); ?>">管理</a>
								</li>
								<?php } ?>
							</ul>
						</div>
						...
						<div class="danceFavoritesList">
							...
							<form method="post" id="list">
								<?php
									...
									$i=0;
									if($classId){
										$sql="select * from ".tname('music')." where CD_Deleted=0 and CD_Passed=0 and CD_UserID='$qianwei_web_userid' and CD_ClassID='$classId' order by CD_AddTime desc";
									}else{
										$sql="select * from ".tname('music')." where CD_Deleted=0 and CD_Passed=0 and CD_UserID='$qianwei_web_userid' order by CD_AddTime desc";
									}
									...
									if($num==0){
										if($classId){
											...
											echo '<div class="nothing_recommend">'.ReplaceStr($qianwei_web_callname,"我","").'还没有上传分享任何音乐。</div>';
										}
									}else{
										if($result){
											echo '<ul class="share_list">';
											echo '<li class="title">';
											echo '<div class="song">音乐</div>';
											echo '<div class="class">类别</div>';
											echo '<div class="source">歌手</div>';
											echo '<div class="time">上传时间</div>';
											...
											while ($row = $db ->fetch_array($result)){
												...
												echo '>';
												echo '<div class="song">';
												echo '<span class="cbox"><input id="'.$row['CD_ID'].'" type="checkbox" value="'.$row['CD_ID'].'" name="did"></span>';
												echo '<div class="aleft">';
												echo '<a href="'.LinkUrl("music",$row['CD_ClassID'],1,$row['CD_ID']).'" target="p" class="mname" onclick="dancePlayer.addPlayer(\''.$row['CD_ID'].'\',\''.$row['CD_UserID'].'\');">'.getlen('len',20,$row["CD_Name"]).'</a>';
												echo '</div>';
												echo '</div>';
												echo '<div class="class">'.GetAlias("qianwei_class","CD_Name","CD_ID",$row['CD_ClassID']).'</div>';
												echo '<div class="source">';
												echo GetSingerAlias("qianwei_singer","CD_Name","CD_ID",$row['CD_SingerID']);
												echo '</div>';
												echo '<div class="time" title="'.date('Y-m-d H:i:s',strtotime($row['CD_AddTime'])).'">'.date("Y-m-d",strtotime($row['CD_AddTime'])).'</div>';
												...
												echo '<span id="d'.$row['CD_ID'].'" class="icon">';
												echo '<b class="default"> </b><!--dislike讨厌  past听过  love喜欢  recommendation推荐-->';
												echo '</span>';
												echo '</div>';
												echo '<div class="action">';
												echo '<a class="add" title="加入列表" href="javascript:;" onclick="dancePlayer.addPlayer(\''.$row['CD_ID'].'\');"></a>';
												echo '</div>';
												echo '<div class="down">';
												echo '<a class="download" title="下载歌曲" href="javascript:;" onclick="dancePlayer.openDown(\''.$row['CD_ID'].'\',\''.$row['CD_UserID'].'\')"></a>';
												echo '</div>';
												echo '</li>';
											}
											echo "</ul>\n";
