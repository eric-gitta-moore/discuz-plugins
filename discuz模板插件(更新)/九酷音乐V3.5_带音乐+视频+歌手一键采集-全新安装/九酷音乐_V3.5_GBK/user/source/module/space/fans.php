...
<head>
	...
	<title><?php echo $qianwei_web_nicheng; ?>的粉丝团</title>
				...
				<div class="stageBoxCenter min_space_height">
					<div id="danceList">
						<div class="spaceMTitle2">
							...
							<ul>
								<li>
									<a href="<?php echo cd_upath.rewrite_url('index.php?p=space&a=following&uid='.$qianwei_web_userid); ?>"><?php echo $qianwei_web_callname; ?>关注的人</a>
									<b></b>
								</li>
								<li class="current">
									<a href="javascript:;"><?php echo $qianwei_web_callname; ?>的粉丝团</a>
									<b></b>
								</li>
								<?php if($qianwei_web_userid == $qianwei_in_userid){ ?>
								<li class="management">
									<a href="<?php echo cd_upath.rewrite_url('index.php?p=relation&a=fans'); ?>">管理</a>
								</li>
								<?php } ?>
							</ul>
						</div>
						...
						<div class="fansList">
							<?php
								global $db;
								$sql = "select * from ".tname('friend')." where cd_hidden=0 and cd_uids='$qianwei_web_userid' order by cd_addtime desc";
								...
								if($result){
									if($num>0){
										...
										while ($row = $db ->fetch_array($result)){
											...
											echo '<a href="'.linkweburl($row['cd_uid'],$row['cd_unames']).'"><img class="avatar-80" width="80" height="80" src="'.getavatar($row['cd_uid'],48).'"/></a>';
											echo '</div>';
											echo '<ul class="info">';
											echo '<li><a title="'.$user['cd_nicheng'].'" href="'.linkweburl($row['cd_uid'],$row['cd_unames']).'">'.$user['cd_nicheng'].'</a>'.CheckCertify($user['cd_checkmusic'],$user['cd_checkmm'],$user['cd_grade'],$user['cd_viprank']).'</li>';
											...
											$sql = "select cd_lock from ".tname('friend')." where cd_uid='$qianwei_in_userid' and cd_uids='".$row['cd_uid']."'";
											if($rows = $db->getrow($sql)){
												if($rows['cd_lock'] == '1'){
													echo '<a onclick="$call(function(){fans.fansDel('.$row['cd_uid'].', \'{'.$user['cd_nicheng'].'}\', 1)});" class="attention mutual" href="javascript:;"></a>';
												}else{
													$cd_groupid = $db->getone("select cd_id from ".tname('friend')." where cd_uid='$qianwei_in_userid' and cd_uids='".$row['cd_uids']."'");
													if($cd_groupid){
														echo '<a onclick="$call(function(){fans.fansDel('.$row['cd_uid'].', \'{'.$user['cd_nicheng'].'}\', 1)});" class="attention already" href="javascript:;"></a>';
													}else{
														echo '<a onclick="$call(function(){fans.fansAdd('.$row['cd_uid'].', \'{'.$user['cd_nicheng'].'}\', 1)});" class="attention" href="javascript:;"></a>';
													}
												}
											}else{
												echo '<a onclick="$call(function(){fans.fansAdd('.$row['cd_uid'].', \'{'.$user['cd_nicheng'].'}\', 1)});" class="attention" href="javascript:;"></a>';
							...
							<?php if($num>0){?>
								<div class="page" id="page">
									<div class="pages">
										<?php echo $Arr[0]; ?>
										<input type="text" maxlength="5" onkeydown="function c(event,val){e = event ? event :(window.event ? window.event : null);if(e.keyCode==13){val = parseInt(val);val=isNaN(val)||(val<=0)?1:val;window.location.href='<?php echo cd_upath.'index.php?p=space&a=fans&uid='.$qianwei_web_userid.'&pages='; ?>'+val;return false;}else{return true;}};if(!c(event,this.value))return false;" title="输入页码，按回车快速跳转"/>
									</div>
								</div>
							<?php } ?>
						</div>
