...
<head>
	...
	<title>全部音乐认证 - <?php echo cd_webname; ?></title>
...
<div class="people_content">
	<div class="title">
		...
		<ul class="right">
			...
			<li class="certification on"><a class="indexList on" sid="list3" href="<?php echo cd_upath.rewrite_url('index.php?p=home&a=producer'); ?>">全部音乐认证</a><b></b></li>
	...
	<div class="content">
		<ul class="people">
			<?php
				$a = 0;
				$sql = "select * from ".tname('user')." where cd_lock=0 and cd_checkmusic=1 order by cd_id desc";
				$Arr = getwebpage($sql,50,"index.php?p=home&a=producer");
				$result = $db->query($Arr[2]);
				$num = $db->num_rows($result);
				if($num==0){
					echo '<div class="nothing">暂时还没有全部音乐认证啊！</div>';
				}else{
					if($result){
