...
<head>
	...
	<title>全部音乐分享人 - <?php echo cd_webname; ?></title>
...
<div class="people_content">
	<div class="title">
		<span>全部音乐分享人</span>
		<ul class="right">
			<li class="share on"><a class="indexList on" sid="list5" href="<?php echo cd_upath.rewrite_url('index.php?p=home&a=usershare'); ?>">全部音乐分享人</a><b></b></li>
	...
	<div class="content">
		<ul class="people">
			<?php
				$a = 0;
				$sql = "select * from ".tname('user')." where cd_lock=0 and cd_checkmusic=0 and cd_musicnum>0 order by cd_id desc";
				$Arr = getwebpage($sql,50,"index.php?p=home&a=usershare");
				$result = $db->query($Arr[2]);
				$num = $db->num_rows($result);
				if($num==0){
					echo '<div class="nothing">暂时还没有全部音乐分享人啊！</div>';
				}else{
					if($result){
