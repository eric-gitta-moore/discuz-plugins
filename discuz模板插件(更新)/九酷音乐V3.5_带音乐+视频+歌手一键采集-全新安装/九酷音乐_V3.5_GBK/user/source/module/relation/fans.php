...
<head>
	...
	<title>我的粉丝 - <?php echo cd_webname; ?></title>
			...
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						...
						<li class="fans">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=relation&a=fans'); ?>">我的粉丝</a>
						</li>
						<li class="praise">
				...
				<div class="minHeight500">
					<div class="relation_userlist">
						<?php
							global $db;
							$sql = "select * from ".tname('friend')." where cd_hidden=0 and cd_uids='$qianwei_in_userid' order by cd_addtime desc";
							...
							if($result){
								if($num>0){
									...
									while ($row = $db ->fetch_array($result)){
										$a=$a+1;
										$user = $db->getrow("select cd_nicheng,cd_favnum,cd_fansnum,cd_introduce,cd_checkmusic,cd_checkmm,cd_grade,cd_viprank from ".tname('user')." where cd_id='".$row['cd_uid']."'");
										...
										echo '<img class="avatar-80" width="80" height="80" src="'.getavatar($row['cd_uid'],48).'"/>';
										echo '</a>';
										echo '</div>';
										echo '<ul class="info">';
										echo '<li><a href="'.linkweburl($row['cd_uid'],$row['cd_uname']).'" target="_blank">'.$user['cd_nicheng'].'</a>'.CheckCertify($user['cd_checkmusic'],$user['cd_checkmm'],$user['cd_grade'],$user['cd_viprank']).'</li>';
										echo '<li>收藏<strong>'.$user['cd_favnum'].'</strong>首 、 粉丝<strong>'.$user['cd_fansnum'].'</strong>人</li>';
										echo '<li>关注时间：'.datetime($row['cd_addtime']).'</li>';
										...
										if($rows = $db->getrow($sql)){
											if($rows['cd_lock'] == '1'){
												...
												$cd_groupid = $db->getone("select cd_id from ".tname('friend')." where cd_uid='$qianwei_in_userid' and cd_uids='".$row['cd_uid']."'");
												if($cd_groupid){
