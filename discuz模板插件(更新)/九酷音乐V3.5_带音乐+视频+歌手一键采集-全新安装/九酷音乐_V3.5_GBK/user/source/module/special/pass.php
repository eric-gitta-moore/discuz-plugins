...
<head>
	...
	<title>待审专辑 - <?php echo cd_webname; ?></title>
			...
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						...
						<li class="find">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=special&a=pass'); ?>">待审专辑</a>
						</li>
					</ul>
					<div class="action">
				...
				<div class="main_nav2">
					<ul>
						<li<?php if($classId == ""){echo " class=\"current\"";} ?>><a href="<?php echo cd_upath.rewrite_url('index.php?p=special&a=pass'); ?>">全部类型</a></li>
						<?php
							global $db;
								...
								echo "><a href=\"".cd_upath.rewrite_url("index.php?p=special&a=pass&classId=".$row["CD_ID"])."\">".$row["CD_Name"]."</a></li>";
							}
						?>
					</ul>
				...
				<div id="imageList" class="minHeight500">
						...
						<?php
							global $db;
							if($classId){
								$sql="select * from ".tname('special')." where CD_Passed=1 and CD_User='$qianwei_in_username' and CD_ClassID='$classId' order by CD_AddTime desc";
							}else{
								$sql="select * from ".tname('special')." where CD_Passed=1 and CD_User='$qianwei_in_username' order by CD_AddTime desc";
							}
							$Arr=getuserpage($sql,20);//sql,每页显示条数
							$result=$db->query($Arr[2]);
							$num=$db->num_rows($result);
							if($num==0){
								echo '<div class="nothing">暂无待审专辑。</div>';
							}else{
								if($result){
									...
									while ($row = $db ->fetch_array($result)){
										...
										echo '<label style="cursor:pointer;">'.getlen("len","6",$row['CD_Name']).'</label>';
...
<script type="text/javascript">listenMsg.start();nav.init();nav.userMenu();specialLib.passDelInit();specialLib.songSetInit();</script>
</body>
</html>