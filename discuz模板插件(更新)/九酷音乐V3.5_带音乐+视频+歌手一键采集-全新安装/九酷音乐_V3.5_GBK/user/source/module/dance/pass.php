...
<head>
	...
	<title>待审音乐 - <?php echo cd_webname; ?></title>
			...
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						...
						<li class="find">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=pass'); ?>">待审音乐</a>
						</li>
					</ul>
					<div class="action">
				...
				<div class="main_nav2">
					<ul>
						<li<?php if($classId == ""){echo " class=\"current\"";} ?>><a href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=pass'); ?>">全部类型</a></li>
						<?php
							global $db;
								...
								echo "><a href=\"".cd_upath.rewrite_url("index.php?p=dance&a=pass&classId=".$row["CD_ID"])."\">".$row["CD_Name"]."</a></li>";
							}
						?>
					</ul>
				...
				<div id="favoritesList" class="minHeight500">
					<div class="private_dance_list">
						...
						<?php
							...
							$a=0;
							if($classId){
								$sql="select * from ".tname('music')." where CD_Passed=1 and CD_UserID='$qianwei_in_userid' and CD_ClassID='$classId' order by CD_AddTime desc";
							}else{
								$sql="select * from ".tname('music')." where CD_Passed=1 and CD_UserID='$qianwei_in_userid' order by CD_AddTime desc";
							}
							$Arr=getuserpage($sql,20);//sql,每页显示条数
							$result=$db->query($Arr[2]);
							$num=$db->num_rows($result);
							if($num==0){
								echo '<div class="nothing">暂无待审音乐。</div>';
							}else{
								if($result){
									echo '<ul id="list">';
									echo '<li class="title">';
									...
									while ($row = $db ->fetch_array($result)){
										...
										echo '<a class="del" did="'.$row['CD_ID'].'" title="删除歌曲" href="javascript:;"></a>';
										echo '</div>';
										echo '</li>';
									}
									echo '</ul>';
...
<script type="text/javascript">listenMsg.start();nav.init();nav.userMenu();danceLib.passDelInit();</script>
</body>
</html>