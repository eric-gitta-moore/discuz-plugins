...
<head>
	...
	<title>我喜欢的音乐 - <?php echo cd_webname; ?></title>
		...
		<div class="user_main">
			<div class="uMain_content">
				<div class="main_nav">
					<ul>
						<li class="like">
							<a class="on" href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=like'); ?>">我喜欢的</a>
						</li>
						<li class="boring">
				...
				<div class="main_nav2">
					<ul>
						<li<?php if($classId == ""){echo " class=\"current\"";} ?>><a href="<?php echo cd_upath.rewrite_url('index.php?p=dance&a=like'); ?>">全部类型</a></li>
						<?php
							global $db;
								...
								echo "><a href=\"".cd_upath.rewrite_url("index.php?p=dance&a=like&classId=".$row["CD_ID"])."\">".$row["CD_Name"]."</a></li>";
							}
						?>
					</ul>
				...
				<div id="favoritesList" class="minHeight500">
					<div class="private_dance_list">
						...
						<?php
							...
							$a=0;
							if($classId){
								$sql="select * from ".tname('like')." where cd_uid='$qianwei_in_userid' and cd_classid='$classId' order by cd_addtime desc";
							}else{
								$sql="select * from ".tname('like')." where cd_uid='$qianwei_in_userid' order by cd_addtime desc";
							}
							$Arr=getuserpage($sql,20);//sql,每页显示条数
							$result=$db->query($Arr[2]);
							$num=$db->num_rows($result);
							if($num==0){
								echo '<div class="nothing">您没有喜欢任何音乐。</div>';
							}else{
								if($result){
									echo '<ul id="list">';
									echo '<li class="title">';
...
<script type="text/javascript">listenMsg.start();nav.init();nav.userMenu();danceLib.likeDelInit();</script>
</body>
</html>